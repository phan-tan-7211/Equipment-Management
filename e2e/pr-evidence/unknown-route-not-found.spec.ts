...
import { expect, test } from '../user/fixtures/CEV.PhanTan-test';
import {
...
test.describe('PR evidence unknown public routes @pr-evidence', () => {
  test('unknown public routes render the public not-found page', async ({ page }) => {
    ...
    await expect(
      page.getByRole('link', { name: /view release notes for CEV.PhanTan version/i }),
  ...
  test('/releases still renders the public releases page', async ({ page }) => {
    ...
    await expect(heading).toBeVisible({ timeout: 20_000 });
    await expect(
      page.getByText(/customer-facing changes in each published CEV.PhanTan release/i),
    ).toBeVisible();
