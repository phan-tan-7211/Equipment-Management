...
test.describe('PR evidence public image layout @pr-evidence', () => {
  ...
  test('landing images load from public/images', async ({ page }) => {
    ...
    await expectPublicImageOk(page, '/images/brand/icons/CEV.PhanTan-Icon-Purple-Small.png');
    ...
    const headerBrand = page.getByRole('banner').getByRole('link', { name: /CEV.PhanTan/i }).first();
...
test.describe('PR evidence dashboard brand icon @pr-evidence', () => {
  test('mobile topbar brand icon loads from public/images', async ({ page, gotoDashboard }) => {
    ...
    await gotoDashboard('/');
    const brand = page.locator('img[src="/images/brand/icons/CEV.PhanTan-Icon-Purple-Small.png"]');
