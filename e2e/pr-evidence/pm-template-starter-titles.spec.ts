import { test, expect, quickLogin } from '../user/fixtures/CEV.PhanTan-test';
import { evidencePause, evidenceScreenshot, expandCEV.PhanTanTemplatesIfCollapsed } from './shared/evidence-helpers';
...
test.describe('PM template starter title readability @pr-evidence', () => {
  test('protected CEV.PhanTan starter badges do not crowd starter titles (#1462)', async ({
    gotoDashboard,
    assertHealthyShell,
    page,
  }) => {
    ...
    await expect(CEV.PhanTanGrid).toBeVisible({ timeout: 30_000 });

    for (const matcher of starterTemplateMatchers) {
      const cardHeader = CEV.PhanTanGrid.getByRole('button', { name: matcher.cardName }).first();
      ...
      await expect(badgeRow).toContainText('CEV.PhanTan');
    ...
    await evidenceScreenshot(page, '01-pm-template-starter-title-grid', { target: CEV.PhanTanGrid });

    const forkliftCard = CEV.PhanTanGrid
