...
/** Default CEV.PhanTan session replay file after local QuickBooks Connect capture. */
...
/** Path where local QuickBooks Connect capture writes CEV.PhanTan session storage state. */
...
export function resolveRealAuthBaseUrl(): string {
  ...
  return raw ? raw.replace(/\/+$/, '') : 'https://preview.CEV.PhanTan.app';
}
