...
export async function pinContextToOrg(
  ...
  await context.addInitScript((orgId) => {
    if (sessionStorage.getItem('CEV.PhanTan_e2e_org_pin_applied') === 'true') {
      return;
    }
    sessionStorage.setItem('CEV.PhanTan_e2e_org_pin_applied', 'true');

    const selectionTimestamp = new Date().toISOString();
    localStorage.setItem('CEV.PhanTan_current_organization', orgId);
    localStorage.setItem(
      'CEV.PhanTan_current_org',
      JSON.stringify({
        selectedOrgId: orgId,
        selectionTimestamp,
      }),
    ...
    localStorage.removeItem('CEV.PhanTan_session_data');
  }, organizationId);
}
