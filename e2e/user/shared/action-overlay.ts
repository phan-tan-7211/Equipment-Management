  ...
  await locator
    .evaluate(
      async (element, options) => {
        const ringId = 'CEV.PhanTan-e2e-action-spotlight';
        const labelId = 'CEV.PhanTan-e2e-action-spotlight-label';
...
export async function installActionOverlay(page: Page, title: string): Promise<void> {
  ...
  await page.addInitScript(({ initialTitle, overlayMode, recordingTitle }) => {
    const overlayId = 'CEV.PhanTan-e2e-action-overlay';
    const ensureOverlay = () => {
      ...
      if (!overlay) {
        ...
        label.textContent = overlayMode === 'marketing' ? 'CEV.PhanTan demo' : 'CEV.PhanTan E2E';
    ...
    (window as Window & { __CEV.PhanTanE2ESetStatus?: (message: string) => void })
      .__CEV.PhanTanE2ESetStatus = setStatus;
    ...
    await page
      .evaluate((status) => {
        const setter = (window as Window & {
          __CEV.PhanTanE2ESetStatus?: (message: string) => void;
        }).__CEV.PhanTanE2ESetStatus;
        setter?.(status);
      }, message)
      .catch(() => undefined);
  }
