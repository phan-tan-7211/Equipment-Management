...
import {
  ...
  waitForCEV.PhanTanDashboardShell,
...
/**
 * Headed one-time capture: sign in to CEV.PhanTan (manual if needed), connect QuickBooks
...
setup('capture QuickBooks local integration @quickbooks-auth-setup', async ({ page }) => {
  ...
  if (page.url().includes('/auth')) {
    ...
    console.log(
      [
        '',
        '[quickbooks-auth-setup] Sign in to CEV.PhanTan in the browser window.',
    ...
    await waitForCEV.PhanTanDashboardShell(page);
  }
  ...
  console.log(
    [
      `[quickbooks-auth-setup] Saved CEV.PhanTan session storage state to ${outputPath}`,
