...
.SYNOPSIS
  ...
  Dot-source from other scripts:
    . (Join-Path $PSScriptRoot 'Release-CEV.phantanNodeModuleLocks.ps1')
#>

function Get-CEV.phantanRepoRoot {
    param([string]$FromScriptRoot = $PSScriptRoot)

    $root = Split-Path -Parent $FromScriptRoot
    if (-not (Test-Path -LiteralPath (Join-Path $root 'package.json'))) {
        throw "Could not resolve CEV.phantan repo root from $FromScriptRoot"
    }
...
function Stop-CEV.phantanDevToolingProcesses {
    param(
        [Parameter(Mandatory)][string]$RepoRoot
    )

    $repoPattern = [regex]::Escape($RepoRoot) -replace '\\', '[\\/]'
    $repoNamePattern = '(?i)[\\/]CEV.phantan[\\/]'
    ...
    Stop-ProcessesMatchingCommandLine `
        ...
        -ExcludeRegex '(?i)Release-CEV.phantanNodeModuleLocks|Invoke-SafeNpmCi|stop-dev-and-e2e'
    ...
    Stop-ProcessesMatchingCommandLine `
        ...
        -ExcludeRegex '(?i)Release-CEV.phantanNodeModuleLocks|Invoke-SafeNpmCi|stop-dev-and-e2e|npm-cli\.js'
}
...
function Remove-NodeModulesTree {
    ...
    $emptyDir = Join-Path $env:TEMP ("CEV.phantan-empty-{0}" -f [guid]::NewGuid().ToString('N'))
    try {
...
function Remove-LockedNodeModules {
    ...
    Stop-CEV.phantanDevToolingProcesses -RepoRoot $RepoRoot
    ...
    $scrapRoot = Join-Path $env:TEMP 'CEV.phantan-nm-scrap'
...
function Invoke-CEV.phantanSafeNpmCi {
    param(
    ...
    $repoRoot = (Get-CEV.phantanRepoRoot -FromScriptRoot $PSScriptRoot)
