...
$token = (op read 'op://CEV.phantan Agents/supabase-write/SUPABASE_ACCESS_TOKEN').Trim()
...
Copy the secret from GCP Console -> CEV.phantan Google Login -> Client secrets, then update production Supabase Auth and app-env-local-dev manually.
"@
}
$workspaceClientId = (op read "op://CEV.phantan Agents/$EdgeItem/GOOGLE_WORKSPACE_CLIENT_ID").Trim()
