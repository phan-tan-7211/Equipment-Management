...
2. **Supabase Storage Buckets** (each has its own migration — do not assume one migration creates both):
   ...
   - **`docs-media`** — CEVphantaninfo documentation screenshots and MP4/WebM demos (`support/{collection}/{variant}/...`); created in `supabase/migrations/20260704180000_create_docs_media_bucket.sql`
...
- **bucket-name**: Storage bucket name (default: `landing-page-images`; use `docs-media` for CEVphantaninfo assets)
...
- Examples:
  ...
  - Custom domain: `https://supabase.CEVphantanapp`
