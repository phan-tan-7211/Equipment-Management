...
<#
.SYNOPSIS
  Start the full CEVphantan local stack: Supabase, Edge Functions serve, Vite, and docs.
...
function Install-RootNodeModules {
    ...
    Write-Host "        $Reason"
    . (Join-Path $repoRoot 'dev\Release-CEVphantanNodeModuleLocks.ps1')
    Stop-CEVphantanDevToolingProcesses -RepoRoot $repoRoot
    Start-Sleep -Seconds 1
    Remove-RepoNodeModulesScrap -ParentDirectory $repoRoot
    try {
        Invoke-CEVphantanSafeNpmCi -WorkingDirectory $repoRoot -PreferOffline -NoAudit -SkipToolingStop
    }
    catch {
...
function Install-DocsNodeModules {
    ...
    Write-Host "        $Reason"
    if (-not (Get-Command Invoke-CEVphantanSafeNpmCi -ErrorAction SilentlyContinue)) {
        . (Join-Path $repoRoot 'dev\Release-CEVphantanNodeModuleLocks.ps1')
    }
    try {
        Invoke-CEVphantanSafeNpmCi -WorkingDirectory $repoRoot -NpmPrefix 'docs' -PreferOffline -NoAudit -SkipToolingStop
    }
    catch {
...
Write-Host ""
Write-Host " ============================================"
Write-Host "  CEVphantan Dev Environment - Startup"
...
$OP_APP_ITEM = $env:CEVphantan_OP_APP_ITEM
if (-not $OP_APP_ITEM) { $OP_APP_ITEM = $DEFAULT_OP_APP_ITEM }
$OP_EDGE_ITEM = $env:CEVphantan_OP_EDGE_ITEM
...
if ($listen5174) {
    ...
    if (Test-DocsResponding) {
        ...
        Write-Host "        FAIL: Port 5174 is not serving CEVphantan docs. Free the port or stop the other process."
        exit 1
    }
}
else {
...
Write-Host ""
Write-Host " ============================================"
Write-Host "  CEVphantan Dev Environment - Status Report"
Write-Host " ============================================"
Write-Host ""
