...
describe('prerenderMarketingHtmlTemplate', () => {
  it('injects work order route copy, nav, metadata, and preserves Vite script', () => {
    ...
    expect(html).toContain(
      '<title>Work Order Management Software for Heavy Equipment Repair | CEVphantan</title>'
    );
    expect(html).toContain(
      '<link rel="canonical" href="https://CEVphantan.app/features/work-order-management" />'
    );
    expect(html).toContain(
  ...
  it('keeps branded releases metadata without appending a second CEVphantan suffix', () => {
    const route = requireMarketingRoute('/releases');

    const html = prerenderMarketingHtmlTemplate(MINIMAL_DIST_TEMPLATE, route);

    expect(html).toContain('<title>Releases · CEVphantan</title>');
    expect(html).toContain('<link rel="canonical" href="https://CEVphantan.app/releases" />');
    expect(html).toContain('Customer-facing changes in each published CEVphantan release.');
    expect(html).not.toContain('Releases · CEVphantan | CEVphantan');
  });
  ...
  it('uses canonical home metadata for the /landing compatibility route', () => {
    ...
    expect(html).toContain(
      '<title>CEVphantan | Free Work Order Software for Heavy Equipment Repair Shops</title>'
    );
    expect(html).toContain('<link rel="canonical" href="https://CEVphantan.app/" />');
    expect(html).toContain(
      '<meta property="og:title" content="CEVphantan | Free Work Order Software for Heavy Equipment Repair Shops" />'
    );
    expect(html).toContain(
      '<meta name="twitter:title" content="CEVphantan | Free Work Order Software for Heavy Equipment Repair Shops" />'
    );
    expect(html).not.toContain('| CEVphantan | CEVphantan');
...
describe('writeAppShellHtml', () => {
  it('writes the untouched Vite template with an empty root for SPA fallback', () => {
    const distDir = mkdtempSync(join(tmpdir(), 'CEVphantan-marketing-'));
    try {
...
describe('writeMarketingHtmlFiles', () => {
  it('preserves app-shell.html before prerendering marketing routes', () => {
    const projectRoot = mkdtempSync(join(tmpdir(), 'CEVphantan-marketing-dist-'));
  ...
  it('keeps app-shell empty, marketing paths prerendered, and no static dashboard file', () => {
    const projectRoot = mkdtempSync(join(tmpdir(), 'CEVphantan-spa-routing-'));
    const distDir = join(projectRoot, 'dist');
    try {
      mkdirSync(distDir, { recursive: true });
      writeFileSync(join(distDir, 'index.html'), MINIMAL_DIST_TEMPLATE, 'utf-8');
  ...
  it('preserves app-shell when Vite emits a root div with attributes', () => {
    ...
    const projectRoot = mkdtempSync(join(tmpdir(), 'CEVphantan-root-attrs-'));
    const distDir = join(projectRoot, 'dist');
    try {
      mkdirSync(distDir, { recursive: true });
