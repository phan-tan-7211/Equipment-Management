...
<#
.SYNOPSIS
  Run CEV.phantan Playwright user regression suites against the local dev stack.
...
if (-not $SkipStackStart -and (-not $appReady -or -not $supabaseReady)) {
    Write-Host "[CEV.phantan E2E] Local stack not ready (app=$appReady supabase=$supabaseReady). Starting dev-start.bat..."
...
if (($Suite -eq 'full' -or $Suite -eq 'all') -and -not $ResetDb) {
    Write-Host '[CEV.phantan E2E] WARNING: full/all suites mutate data; auto-enabling -ResetDb for repeatable runs.'
    $ResetDb = $true
}

if ($ResetDb) {
    Write-Host '[CEV.phantan E2E] Generating volume seed data (dev/seed-data/generate-seeds.ts)...'
    npx tsx dev/seed-data/generate-seeds.ts
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    Write-Host '[CEV.phantan E2E] Resetting local database (supabase db reset)...'
...
function Read-RunConfigDefaults {
    ...
    try {
        ...
        Write-Host "[CEV.phantan E2E] WARNING: Could not read $defaultRunConfigPath; using built-in defaults."
    }
...
function Invoke-PlaywrightViewportRun {
    ...
    Write-Host "[CEV.phantan E2E] Running suite: $Suite"
    Write-Host "[CEV.phantan E2E] Run profile: $resolvedRunProfile"
    Write-Host "[CEV.phantan E2E] Effective config: $runConfigPath"
    Write-Host "[CEV.phantan E2E] Viewport mode: $Viewport"
    Write-Host "[CEV.phantan E2E] Output folder: $absoluteOutputDir"
    if ($effectiveWatch) {
        Write-Host "[CEV.phantan E2E] Watch pacing: overlay=$resolvedOverlayMode, stagePause=$resolvedStagePauseMs ms, finalPause=$resolvedWatchPauseMs ms, slowMo=$resolvedSlowMoMs ms"
    }
    if ($recordAllVideos) {
        Write-Host "[CEV.phantan E2E] Recording videos for every test under tmp\playwright\test-results"
    }
    if ($actionCue) {
        Write-Host "[CEV.phantan E2E] Demo action spotlight enabled."
    }
    if ($resolvedOverlayMode -eq 'marketing') {
        Write-Host "[CEV.phantan E2E] Marketing overlay enabled; Playwright video annotations are suppressed."
        if ($resolvedRecordingTitle) {
            Write-Host "[CEV.phantan E2E] Recording title: $resolvedRecordingTitle"
        }
    } elseif ($resolvedOverlayMode -eq 'debug') {
        Write-Host "[CEV.phantan E2E] Debug overlay enabled; Playwright video annotations are enabled."
    } else {
        Write-Host "[CEV.phantan E2E] Overlay disabled."
    }
    Write-Host "[CEV.phantan E2E] Command: npx $($playwrightArgs -join ' ')"
    ...
    if ($runExitCode -ne 0) {
        ...
        if (Test-Path -LiteralPath $report) {
            Write-Host "[CEV.phantan E2E] HTML report: $report"
        }
    }

    if ($recordAllVideos) {
        $videoFiles = @(Get-ChildItem -LiteralPath $absoluteOutputDir -Recurse -Filter 'video.webm' -File -ErrorAction SilentlyContinue)
        Write-Host "[CEV.phantan E2E] Video files retained: $($videoFiles.Count)"
        if ($videoFiles.Count -gt 0) {
            Write-Host "[CEV.phantan E2E] First video: $($videoFiles[0].FullName)"
        }
    }
