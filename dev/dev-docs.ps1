...
Write-Host "[CEVphantan] Docs dev server starting"
...
if ($listen5174 -and (Test-DocsResponding)) {
    ...
    Write-Host "[CEVphantan] Docs ready"
    exit 0
}

if ($listen5174 -and -not (Test-DocsResponding)) {
    Write-Host "FAIL: Port 5174 is in use but not serving CEVphantan docs. Free the port or run dev-stop."
    exit 1
}
