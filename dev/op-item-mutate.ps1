...
.DESCRIPTION
  ...
  Uses OP_SAT_CEV.phantan (write) by default. Pass -ReadOnly to use OP_SERVICE_ACCOUNT_TOKEN
...
.PARAMETER Vault
  Vault name or ID (default: CEV.phantan Agents vault ID)
...
function Get-OpServiceAccountToken {
    param([switch]$Write)
    if ($Write) {
        $token = [Environment]::GetEnvironmentVariable('OP_SAT_CEV.phantan', 'User')
        if (-not $token) {
            throw 'OP_SAT_CEV.phantan is not set in User scope. Provision write SAT for CEV.phantan Agents vault.'
        }
        return $token
    }
