...
describe('buildSitemapXml', () => {
  ...
  it('contains the canonical home URL exactly once', () => {
    const matches = sitemap.match(/<loc>https:\/\/CEV.PhanTan\.app\/<\/loc>/g);
  ...
  it('includes representative canonical feature URLs', () => {
    expect(sitemap).toContain('<loc>https://CEV.PhanTan.app/features/work-order-management</loc>');
    expect(sitemap).toContain('<loc>https://CEV.PhanTan.app/features/quickbooks</loc>');
    expect(sitemap).toContain('<loc>https://CEV.PhanTan.app/features/qr-code-integration</loc>');
    expect(sitemap).toContain('<loc>https://CEV.PhanTan.app/releases</loc>');
    expect(sitemap).toContain('<loc>https://CEV.PhanTan.app/right-to-repair</loc>');
  });
  ...
  it('de-dupes routes with the same canonical path (first occurrence wins)', () => {
    ...
    const homeMatches = xml.match(/<loc>https:\/\/CEV.PhanTan\.app\/<\/loc>/g);
