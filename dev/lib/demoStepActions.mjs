...
function demoCuePrelude(label) {
  ...
  return `
      const __CEVphantanDemoCue = async (element) => {
        const ringId = 'CEVphantan-demo-action-spotlight';
        const labelId = 'CEVphantan-demo-action-spotlight-label';
  ...
  async function clickWithSelectorStrategy(role, name, fallbackSelectors, context) {
    ...
    const script = `async () => {
      ...
      if (byRole) {
        ...
        await __CEVphantanDemoCue(byRole);
        byRole.click();
        return;
      }
      ...
      for (const selector of selectors) {
        ...
        if (fallbackMatch) {
          ...
          await __CEVphantanDemoCue(fallbackMatch);
          fallbackMatch.click();
          return;
        }
      }
    ...
    switch (actionName) {
      ...
      case 'clickText': {
        ...
        await withRetries(context.sceneId, context.stepIndex, step, async () => {
          await runPlaywrightEval(
            `async () => { ${demoCuePrelude(`Click ${rawText}`)} const el = Array.from(document.querySelectorAll('${selector}')).find((node) => new RegExp('${text}', 'i').test(node.textContent || '') && !node.hasAttribute('disabled')); if (!el) throw new Error('clickText missing element'); await __CEVphantanDemoCue(el); el.click(); }`,
          );
        });
      ...
      case 'fillRole': {
        ...
        await withRetries(context.sceneId, context.stepIndex, step, async () => {
          const output = await runPlaywrightEval(`async () => {
            ...
            await __CEVphantanDemoCue(input);
            input.focus();
            input.value = '${value}';
      ...
      case 'selectOption': {
        ...
        const value = jsString(String(step.value || ''));
        await runPlaywrightEval(
          `async () => { ${demoCuePrelude(`Select ${String(step.value || '')}`)} const el = document.querySelector('${selector}'); if (!el) throw new Error('selectOption missing select'); await __CEVphantanDemoCue(el); el.value = '${value}'; el.dispatchEvent(new Event('change', { bubbles: true })); }`,
        );
        recordSpotlight(context.sceneId, context.stepIndex, actionName);
        context.actionCountRef.value += 1;
        return;
      ...
      case 'openDialog': {
        ...
        await runPlaywrightEval(
          `async () => { ${demoCuePrelude(actionName.replace(/^open/i, 'Open '))} const el = document.querySelector('${selector}'); if (!el) throw new Error('open action missing selector'); await __CEVphantanDemoCue(el); el.click(); }`,
        );
        recordSpotlight(context.sceneId, context.stepIndex, actionName);
        context.actionCountRef.value += 1;
        return;
      ...
      case 'focusElement': {
        ...
        await runPlaywrightEval(
          `async () => { ${demoCuePrelude('Focus field')} const el = document.querySelector('${selector}'); if (!el) throw new Error('focusElement missing selector'); await __CEVphantanDemoCue(el); el.focus(); }`,
        );
        recordSpotlight(context.sceneId, context.stepIndex, actionName);
        context.actionCountRef.value += 1;
        return;
      ...
      case 'hoverReveal': {
        ...
        await runPlaywrightEval(
          `async () => { ${demoCuePrelude('Reveal actions')} const el = document.querySelector('${selector}'); if (!el) throw new Error('hoverReveal missing selector'); await __CEVphantanDemoCue(el); el.dispatchEvent(new MouseEvent('mouseover', { bubbles: true })); }`,
        );
        recordSpotlight(context.sceneId, context.stepIndex, actionName);
        context.actionCountRef.value += 1;
        return;
      ...
      case 'clickHiddenButton': {
        ...
        await withRetries(context.sceneId, context.stepIndex, step, async () => {
          await runPlaywrightEval(`async () => {
            ...
            await __CEVphantanDemoCue(button);
            button.click();
          }`);
        });
        recordSpotlight(context.sceneId, context.stepIndex, actionName);
