...
describe('parseDemoGifArgs', () => {
  ...
  it('parses --base-url and strips trailing slashes', () => {
    const parsed = parseDemoGifArgs(['--smoke', '--base-url=https://CEVphantan.app/']);
    expect(parsed.baseUrl).toBe('https://CEVphantan.app');
  });
...
describe('isLocalhostBaseUrl', () => {
  it('detects localhost', () => {
    ...
    expect(isLocalhostBaseUrl('https://CEVphantan.app')).toBe(false);
  });
});
