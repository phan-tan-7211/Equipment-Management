...
<#
.SYNOPSIS
  Sync Vercel project env vars from the CEV.phantan Agents 1Password vault.
...
.DESCRIPTION
  ...
  blob) so each can be referenced via op://CEV.phantan Agents/{item}/{field}.
...
$OP_VAULT = 'tgo2m6qbct5otqeqirjocn3joa'  # CEV.phantan Agents
...
foreach ($envName in $envsToProcess) {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Warn "1Password item '$opItem' not found in CEV.phantan Agents vault. Skipping $envName."
