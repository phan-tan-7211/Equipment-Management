...
# CEVphantanAgents vault UUID (spaced names break some op CLI paths).
OP_CEVphantanAGENTS_VAULT_ID="${CLOUD_AGENT_OP_VAULT_ID:-tgo2m6qbct5otqeqirjocn3joa}"
...
ca_load_supabase_access_token() {
  ...
  local token
  token="$(op read "op://${OP_CEVphantanAGENTS_VAULT_ID}/supabase-write/SUPABASE_ACCESS_TOKEN" 2>/dev/null || true)"
  token="$(printf '%s' "$token" | tr -d '\r\n')"
  if [[ -z "$token" ]]; then
    ca_fail "Could not read op://${OP_CEVphantanAGENTS_VAULT_ID}/supabase-write/SUPABASE_ACCESS_TOKEN"
    return 1
  fi
