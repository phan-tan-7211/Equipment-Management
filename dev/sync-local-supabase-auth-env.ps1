...
function Read-OpField {
    ...
    try {
        return (op read "op://CEV.phantan Agents/$Item/$Field").Trim()
    } catch {
        return $null
    }
}
...
$appItem = if ($env:CEV.phantan_OP_APP_ITEM) { $env:CEV.phantan_OP_APP_ITEM } else { 'app-env-local-dev' }
...
$lines = @(
    "# CEV.phantan local Supabase Auth (Google sign-in) - synced from 1Password item $appItem"
