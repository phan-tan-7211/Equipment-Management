...
function Invoke-CreatePullReviewCommentReply {
    ...
    $tmp = Join-Path $env:TEMP ("CEVphantan-pr-reply-{0}.json" -f [Guid]::NewGuid().ToString('N'))
    try {
...
if ($DeferredIssuesFile) {
    ...
    foreach ($it in $items) {
        ...
        $tmpBody = Join-Path $env:TEMP ("CEVphantan-issue-body-{0}.md" -f [Guid]::NewGuid().ToString('N'))
        try {
            $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
