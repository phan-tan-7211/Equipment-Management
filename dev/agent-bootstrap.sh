#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# CEVphantan— Cursor Cloud Agent bootstrap (Linux)
#
...
#      CEVphantanAgents 1Password vault.
...
# ──────────────────────────────────────────────────────────────────────────────
# 3. Render local app .env from CEVphantanAgents vault
# ──────────────────────────────────────────────────────────────────────────────
log "[3/5] Rendering .env files from CEVphantanAgents vault..."

OP_VAULT='tgo2m6qbct5otqeqirjocn3joa'  # CEVphantanAgents
...
render_section_item() {
    ...
    if ! op item get "$item_name" --vault "$OP_VAULT" --format json > "$item_json" 2>/dev/null; then
        ...
        warn "1Password item '${item_name}' not found in CEVphantanAgents vault."
...
GCP_KEY_PATH="${GCP_KEY_DIR}/CEVphantanagent-viewer.json"
GCP_ITEM_NAME=""
...
NODE
    ...
    warn "Neither 1Password item 'gcp-read' nor legacy item 'gcp-viewer' was found in CEVphantanAgents vault. gcloud MCP will fail until the viewer service-account JSON is present."
fi
