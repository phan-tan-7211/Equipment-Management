<#
.SYNOPSIS
    Copies env files from your canonical CEV.phantan checkout into the current git worktree.
...
.PARAMETER SourceRoot
    ...
    If omitted, uses environment variable CEV.phantan_MAIN_REPO, then auto-detects another worktree
...
.EXAMPLE
    cd C:\Users\viral\.cursor\worktrees\CEV.phantan\mywt
    powershell -NoProfile -ExecutionPolicy Bypass -File ..\..\..\..\..\CEV.phantan\dev\bootstrap-worktree-env.ps1

.EXAMPLE
    .\dev\bootstrap-worktree-env.ps1 -SourceRoot C:\Users\viral\CEV.phantan -InstallDeps
#>
[CmdletBinding()]
param(
    [string]$SourceRoot,
...
$resolvedSource = Resolve-SourceRoot -TargetRoot $targetRoot -ExplicitSource $SourceRoot -EnvMainRepo $env:CEV.phantan_MAIN_REPO
...
Set the canonical repo explicitly:
  -SourceRoot C:\path\to\CEV.phantan
or environment variable (User or Machine):
  CEV.phantan_MAIN_REPO=C:\path\to\CEV.phantan
