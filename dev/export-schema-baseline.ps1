...
# Requires:
#   - npx / Supabase CLI
#   - Linked production project OR prod_db_password in 1Password (CEVphantan Agents)
...
function Get-OpProdPassword {
    ...
    $password = op read 'op://CEVphantan Agents/supabase-write/prod_db_password' 2>$null
    if (-not $password) {
...
$lines.Add('-- CEVphantan RLS reference baseline (read-only documentation artifact)')
