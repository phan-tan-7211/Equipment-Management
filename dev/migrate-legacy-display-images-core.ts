...
export interface MigrationStorage {
  ...
  remove(bucket: 'display-images', paths: string[]): Promise<void>;
  stat(
    bucket: 'display-images',
    path: string,
...
export async function runLegacyDisplayImageMigration(
  ...
  for (const candidate of page.rows) {
    ...
    try {
      ...
      let canonicalRefCommitted = false;

      try {
        for (const variant of DISPLAY_IMAGE_VARIANT_NAMES) {
          const result = await dependencies.storage.upload(
            'display-images',
            variantPaths[variant],
            variants[variant],
          );
          if (result === 'uploaded') {
            newlyUploadedPaths.push(variantPaths[variant]);
          }
        }
        ...
        canonicalRefCommitted = true;
        report.migrated += 1;
        report.bytesAfter += bytesAfter;
        report.bytesAfterMeasured += 1;
      } finally {
        if (!canonicalRefCommitted && newlyUploadedPaths.length > 0) {
          try {
            ...
            // The report retains the primary failure. A later V2 orphan audit
            // can retry cleanup without touching the legacy source or DB ref.
          }
        }
      }
    } catch (error) {
