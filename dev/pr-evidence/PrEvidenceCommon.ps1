...
function Set-PrEvidenceUploadEnvironment {
    ...
    $urlRead = Invoke-PrEvidenceNative -FilePath 'op' -Arguments @(
        'read', 'op://CEV.phantan Agents/app-env-preview-public/SUPABASE_URL'
    )
    ...
    $keyRead = Invoke-PrEvidenceNative -FilePath 'op' -Arguments @(
        'read', 'op://CEV.phantan Agents/supabase-write/prod_service_role_key'
    )
...
function Publish-PrEvidenceGitHubVideo {
    param(
        [Parameter(Mandatory)][string]$Mp4Path,
        [string]$Repo = 'Columbia-Cloudworks-LLC/CEV.phantan'
    )
...
function Write-PrEvidenceVisualReviewChecklist {
    ...
    $content = @(
        ...
        '- Layout looks **intentionally designed** for CEV.phantan — not merely functional enough for now.',
        '',
        '## Screenshots',
        '',
        '| # | Label | Local path |',
