...
| `Publish-DocsMedia.ps1` | Upload capture artifacts to public `docs-media` for CEVphantan.info articles |

## Documentation media bucket

For CEVphantan.info (not PR comments), publish capture manifests to **`docs-media`**:
...
- **`GH_SESSION_TOKEN` (User scope)** for GitHub inline video upload — a GitHub `user_session` cookie, **not** a PAT. One-time setup:
  ...
  gh image extract-token | Set-Content -Path "$env:USERPROFILE\.CEVphantan-gh-session-token" -NoNewline -Encoding ascii
  [Environment]::SetEnvironmentVariable('GH_SESSION_TOKEN', (Get-Content "$env:USERPROFILE\.CEVphantan-gh-session-token" -Raw).Trim(), 'User')
  ```
