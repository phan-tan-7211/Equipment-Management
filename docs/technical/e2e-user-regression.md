...
From Cursor / VS Code, open **Tasks: Run Task** (`Ctrl+Shift+P` -> "Run Task") and pick any **CEVphantan E2E ...** entry (critical, full, watch, record, support-record, reset-db, inspector, or Chromium install). These wrap `dev-test.bat` with the same flags as the examples below. To cancel a run and tear down the stack, use **CCEVphantanStop E2E Tests and Dev Stack** (or `.\dev\dev-stop-all.bat`).
