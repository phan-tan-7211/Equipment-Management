...
This guide combines quick start instructions, environment configuration, and troubleshooting to help you get up and running with CEVphantan development quickly.
...
If you have access to the CEVphantanAgents 1Password vault, use `dev-start.bat` as the default setup path. It syncs app `.env` from the editable `app-env-local-dev` item and Edge Function `supabase/functions/.env` from the editable `edge-env-local-dev` item automatically.
...
# for the read-only op-svc-CEVphantanagents service account, then refresh in-session:
...
CEVphantanuses three categories of environment variables:
...
👉 **[Supabase Branch Secrets Configuration](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/supabase-branch-secrets.md)**
...
See **[Local Supabase Development Guide](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/local-supabase-development.md)** for complete setup instructions.

### Setting Up Supabase

> **⚠️ IMPORTANT: Local Supabase is the standard development method.**
> 
> All database development should be done locally first, then deployed to production. See [Local Supabase Development Guide](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/local-supabase-development.md) for complete setup instructions.
...
- **Node.js** — must satisfy `engines.node` in the root [`package.json`](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/package.json) (currently **Node 24.x LTS**). We recommend the latest **24.x** LTS release for local development. [Download here](https://nodejs.org/)
...
**Workflow:** Keep one canonical clone (for example `C:\Users\viral\CEVphantan) where you run `.\dev\dev-start.bat` so env files exist on disk. In any other worktree, bootstrap once:
...
```powershell
$env:CEVphantanMAIN_REPO = "C:\Users\viral\CCEVphantan  # optional persistent default
.\dev\bootstrap-worktree-env.ps1 -SourceRoot "C:\Users\viral\CEVphantan -InstallDeps
```
...
**Note**: See [Local Supabase Development Guide](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/local-supabase-development.md) for detailed setup instructions.
...
Welcome to the CEVphantan development team!
