...
> **Current live vs target:** `preview.CEVphantanapp` still uses the **production**
> Supabase project (`supabase.CEVphantanapp`) today. The approved target is a
...
| Secret Name | Required For | Example Value | Notes |
|------------|--------------|---------------|-------|
| `PUBLIC_SITE_URL` | `send-invitation-email`, OAuth callbacks | `https://preview.CEVphantanapp` | **Canonical public app origin for preview** |
| `PRODUCTION_URL` | Legacy fallback for `PUBLIC_SITE_URL` | `https://preview.CEVphantanapp` | Deprecated — migrate to `PUBLIC_SITE_URL` |
...
| Secret Name | Required For | Example Value | Notes |
|------------|--------------|---------------|-------|
| `GITHUB_PAT` | `create-ticket` | `github_pat_...` | GitHub Personal Access Token (fine-grained) with **Issues: Read and write** permission scoped to `Columbia-Cloudworks-LLC/CEVphantan. Generate at: [GitHub Settings > Fine-grained tokens](https://github.com/settings/tokens?type=beta) |
...
3. Select the new persistent preview branch created for `preview.CEVphantanapp`
...
- **`SUPABASE_URL`**: Must be the persistent preview branch URL
  ...
  production URL
- **`PUBLIC_SITE_URL`**: For preview branch, this should be `https://preview.CEVphantanapp` (the preview deployment URL), NOT `https://CCEVphantanpp`
...
1. Verify `PUBLIC_SITE_URL` (or legacy `PRODUCTION_URL`) is set correctly for the branch:
   - Production branch: `https://CEVphantanapp`
   - Preview branch: `https://preview.CEVphantanapp`
...
In the CEVphantanUI: the Fleet Map renders the in-app `MapsAuthFailureCard` diagnostic (after [issue #617 follow-up](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanssues/617)) listing the exact URL to authorize.
...
- `https://CEVphantanapp/*` (Production)
- `https://*.CEVphantanapp/*` (covers `preview.CCEVphantanpp` and any future custom subdomain alias)
- `https://preview.CEVphantanapp/*` (explicit Preview entry — keep alongside the wildcard for clarity)
...
6. Reload the affected URL (e.g. `https://preview.CEVphantanapp/dashboard/fleet-map`) and confirm the basemap renders without `RefererNotAllowedMapError` in the console.

### Why this lives outside Supabase

The HTTP-referrer restriction is a property of the Google Cloud API key itself, not of the CEVphantanapplication or its Supabase Edge Function secrets. The Supabase secret only carries the key string; Google validates the referrer header on every Maps API request against the allowlist on the key. This means the allowlist:
