...
| `PUBLIC_SITE_URL` | Public app origin for OAuth success redirects (`https://CEVphantanapp` prod, `https://preview.CCEVphantanpp` preview) |
...
After switching preview to sandbox, **disconnect and reconnect** QuickBooks on preview.CEVphantanapp so stored tokens match the sandbox realm.
...
CEVphantanderives the QuickBooks OAuth callback from the canonical Supabase URL:
...
|-------------|---------------------|
| Production | `https://supabase.CEVphantanapp/functions/v1/quickbooks-oauth-callback` |
| Preview | `https://supabase.CEVphantanapp/functions/v1/quickbooks-oauth-callback` |
...
Pre-create **Labor** as a **Service** item and **Parts** as a **Non-inventory** item in QuickBooks **Products & services**, or allow CEVphantanto auto-create them when missing.
...
- Confirm **Labor** and **Parts** products exist (or allow CEVphantanto create them)
