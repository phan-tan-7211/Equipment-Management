...
This document outlines the disaster recovery procedures for CEVphantan, including how to restore the Supabase database from backups and point-in-time recovery (PITR).
...
CEVphantan uses Supabase as its backend platform, which provides robust backup and recovery capabilities:

| Environment | Project Ref | Purpose |
|-------------|-------------|---------|
| **Production** | `ymxkzronkhwxzcdcbnwq` | Live production database (also serves cloud preview via `supabase.CEVphantanapp`) |
...
curl https://supabase.CEVphantanapp/functions/v1/healthcheck

# Check the public status page for aggregate service health
# https://status.CEVphantanapp
```
...
**Document Owner**: CEVphantan DevOps Team  
