...
Solo-developer workflow for CEVphantanafter #1282 restored the feat → preview → main train.
...
| **`preview`** | Integration / pre-production train. Default merge target for feature work. Deploys to **`preview.CEVphantanapp`**. |
...
| URL | Meaning |
|-----|---------|
| **<https://CEVphantanapp>** | Production (after Production Release Readiness + `vercel promote`) |
| **`https://<project>-<hash>-columbia-cloudworks-llc.vercel.app`** | Commit-specific Vercel Preview URL for every work-branch / PR deploy |
| **<https://preview.CEVphantanapp>** | Stable hostname for the **integration** git branch **`preview`** — Vercel Preview deploys on merges/pushes to that branch (branch-bound custom domain). Not fast-forwarded from `main`. |
...
6. Merge to `preview` → Vercel updates **`preview.CEVphantanapp`**.
7. When ready to ship: **`/release`** or open **`preview` → `main`** with version bump + empty Unreleased → **Production Release Readiness** → **`vercel promote`** → **CEVphantanapp**.
...
| **Preview** env | Branch tracking: enabled for work branches. Custom domain **`preview.CEVphantanapp`** assigned to git branch **`preview`** (normal deploys on push/merge to that branch). |
...
- **Cloud app (`preview.CEVphantanapp` and `CCEVphantanpp`):** current live state is a
  single production project (`https://supabase.CEVphantanapp`). The approved
  target is to move `preview.CEVphantanapp` to a new persistent dataless branch
