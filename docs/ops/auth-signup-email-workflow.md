# CEVphantanProduction Signup Email Workflow
...
| --- | --- |
| App URL | `https://CEVphantanapp` |
| Supabase project | `ymxkzronkhwxzcdcbnwq` |
| Supabase Auth URL | `https://supabase.CEVphantanapp/auth/v1` |
| Supabase Auth email sender | `CEVphantan<noreply@CCEVphantanpp>` |
| SMTP provider | Resend |
| SMTP host | `smtp.resend.com` |
| SMTP port | `587` |
| SMTP username | `resend` |
| Resend sending domain | `CEVphantanapp` |
...
```mermaid
flowchart TD
  A[Visitor opens CEVphantanapp] --> B[Visitor chooses Sign Up]
  ...
  D --> E[POST supabase.CEVphantanapp/auth/v1/signup]
...
1. A user visits `https://CEVphantanapp` and opens the signup tab.
...
4. `signUpWithEmail()` calls `supabase.auth.signUp()` with:
   ...
   - redirect URL `https://CEVphantanapp/`,
...
1. Confirm Supabase Auth config:
   ...
   - `smtp_port = 587`
   - `smtp_admin_email = noreply@CEVphantanapp`
   - `smtp_sender_name = CEVphantan
   - `rate_limit_email_sent = 30`
   - `mailer_autoconfirm = false`
2. Confirm Resend domain health:
   - `CEVphantanapp` is verified.
   - Sending is enabled.
