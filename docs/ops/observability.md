...
> Operational reference for where Google Workspace and Google Cloud audit logs live, how to enable them, and how to query them. Created in response to [issue #629](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/629), which surfaced a recurring confusion about Workspace-tier vs. project-tier audit logging.

## Summary

| Audit log family | Tier | Producer | Where to query |
|---|---|---|---|
| Google Cloud Admin Activity (project services) | Project | Cloud-native services on `CEVphantanprod` | `gcloud logging read --project=CCEVphantanrod` against `projects/CECEVphantanod/logs/cloudaudit.googleapis.com%2Factivity` |
| Google Cloud Data Access (project services) | Project | Cloud-native services on `CEVphantanprod`, **only when enabled** via project `auditConfigs` | `gcloud logging read --project=CCEVphantanrod` against `projects/CECEVphantanod/logs/cloudaudit.googleapis.com%2Fdata_access` |
...
The single most common confusion (the one that produced [#629](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/629)) is between the second and third rows. The remediation is documented below.
...
- **Project-level `gcloud projects set-iam-policy CEVphantanprod` with an `auditConfigs` block on these names is a verified no-op.** The IAM policy will accept and persist the YAML, but no entries will appear under `projects/CCEVphantanrod/logs/cloudaudit.googleapis.com%2Fdata_access` for these service names because the underlying producers do not write at the project tier.
- The Cloud Console **IAM & Admin → Audit Logs** table (`https://console.cloud.google.com/iam-admin/audit?project=CEVphantanprod`) does not list these services as configurable rows for the same reason.
...
This is a Google Workspace tenant action performed by a **Workspace super-admin** on `columbiacloudworks.com`. It is **not** a Google Cloud Console action and cannot be performed by service accounts (including the CEVphantanCursor agent SAs).
...
- [`.env.example` (repository)](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/.env.example) — Workspace OAuth env vars (`VITE_GOOGLE_WORKSPACE_CLIENT_ID`, `GOOGLE_WORKSPACE_CLIENT_ID`, `GOOGLE_WORKSPACE_CLIENT_SECRET`).
- [Issue #629](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/629) — original ticket and redirect comment.
- [Issue #601](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/601) — the customer report that surfaced the original observability gap.
