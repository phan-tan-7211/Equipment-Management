...
When gcloud MCP impersonation is insufficient (OAuth client redirect URIs, Workspace user security, Admin Console policy), agents may sign in through the browser using ZNT Agents vault item `Google (Business)`:
