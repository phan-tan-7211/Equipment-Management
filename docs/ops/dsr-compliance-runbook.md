...
This runbook covers the operational procedures for handling CCPA/CPRA Data Subject Requests in CEVphantan All DSR handling must follow these procedures to ensure lawful compliance and produce auditable evidence.
...
| `opt_out` | Right to Opt-Out | Opt out of sale/sharing of PI (CEVphantandoes not sell PI) |
| `limit_use` | Right to Limit | Limit use of sensitive personal information |

## Intake

1. Consumer submits via `/privacy-request` form or emails `privacy@CEVphantanapp`.
...
CEVphantandoes not sell or share personal information. Acknowledge receipt, confirm no action required, and close with a note explaining the basis.
