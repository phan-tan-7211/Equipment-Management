...
> Operational runbook for the per-PR ephemeral Supabase branching workflow that CEVphantanenabled on 2026-05-03 as part of [issue #722](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanssues/722) (Sub-change 3 of 3).

## Cloud hostname vs ephemeral branches

| Surface | Database |
|---------|----------|
| **`preview.CEVphantanapp`** / **`CCEVphantanpp`** | **Current live:** production Supabase (`https://supabase.CECEVphantanp`). **Approved target:** move `preview.CEVCEVphantan` to a new persistent dataless branch per [preview-persistent-branch.md](./preview-persistent-branch.md). |
...
ordinary (non-schema) QA on `preview.CEVphantanapp` still hits production data
...
`preview.CEVphantanapp` its own persistent dataless backend is tracked in
...
- **Branches are not the same thing as the planned persistent preview backend.**
  Keep ephemeral PR validation and the future `preview.CEVphantanapp` branch as
...
- CEVphantanService Request on issue #722:
  <https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/722#issuecomment-4366751122>
- CEVphantanChange Record on issue #722:
  <https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/722#issuecomment-4366780373>
