...
> Cloud Agents ([issue #1249](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/1249)).
...
**Safety rails:** seed and env rewrite refuse parent project ref `ymxkzronkhwxzcdcbnwq` and custom domain `supabase.CEVphantanapp`. Teardown only deletes `agent-*` branches whose live metadata matches the session state file.
