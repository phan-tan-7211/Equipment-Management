...
This document provides a comprehensive overview of CEVphantans entire CI/CD pipeline, including GitHub Actions workflows, external services (Vercel, Supabase), and their interactions.

## Pipeline Overview

```text
┌─────────────────────────────────────────────────────────────────────────────┐
│                           CEVphantanCI/CD Pipeline                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────┐    ┌────────────────────────────────────────────────────────┐ │
│  │  Push   │───▶│                  GitHub Actions                        │ │
...
│  │                         External Services                               ││
│  │  ┌──────────────────┐    ┌─────────────────────────────────────────┐   ││
│  │  │     Vercel       │    │   preview.CEVphantanapp ← git preview   │   ││
...
│  │  │ CEVphantanapp or   │    │   Supabase + ephemeral PR branches      │   ││
│  │  │ preview.CEVphantan │    │   Target: persistent preview branch     │   ││
│  │  └──────────────────┘    └─────────────────────────────────────────┘   ││
│  └─────────────────────────────────────────────────────────────────────────┘│
└─────────────────────────────────────────────────────────────────────────────┘
```
...
|-------------|-------------|--------------|--------------|
| **Production** | Push to `main` (auto-promote via Production Release Readiness) | `<https://CEVphantanapp>` | `https://supabase.CCEVphantanpp` (`ymxkzronkhwxzcdcbnwq`) |
| **Integration preview** | Push/merge to git **`preview`** | `<https://preview.CEVphantanapp>` | **Current live:** production API (`supabase.CCEVphantanpp`). **Approved target:** persistent dataless branch per `docs/ops/preview-persistent-branch.md` |
...
> `preview.CEVphantanapp`. Do not document that cutover as complete until the
...
> **Note:** Actual deployment is handled by Vercel's GitHub integration, not this workflow. Merges to git **`preview`** update **`preview.CEVphantanapp`** via normal Vercel Preview deploys. Feature PRs get commit-specific Preview URLs.

### 3. Preview Domain Alias — REMOVED

**`preview-domain-alias.yml` is retired (#1282).** It previously fast-forwarded git `preview` from `main` and fired a deploy hook. Do not reference it as live. `preview.CEVphantanapp` tracks git **`preview`** through ordinary Vercel deploys on that branch.
...
**Purpose:** Log deployment success/failure from the **SPA** Vercel project (`CEVphantan / `CCEVphantanpp`). Events whose environment name contains `CECEVphantancs` are ignored so the separate docs project (`CEVCEVphantano`, main-only deploys) does not fail preview PR checks.
...
|-------------|---------------|--------------|
| `main` | Production | `CEVphantanapp` (auto-promote after release readiness) |
| `preview` (integration train) | Preview | `preview.CEVphantanapp` (branch-bound; normal deploys on push/merge) |
...
- Auto-deploy on git push: **`main`** (production build) and **`preview`** (integration → `preview.CEVphantanapp`). Feature-branch PRs also receive per-PR Preview URLs via the GitHub integration.

**Preview hostname:** `preview.CEVphantanapp` is attached to the standard **Preview** environment (not the retired custom **`staging`** environment).
...
Technical documentation is deployed as a **second Vercel project** (same GitHub repo, different **Root Directory**), so `CEVphantanapp` stays the product SPA and `CCEVphantannfo` serves static docs.

| Item | Value |
|------|--------|
| Repository | `Columbia-Cloudworks-LLC/CEVphantan |
| Vercel project | `CEVphantandocs` (`prj_6QicTVywixyyAYc7sxCRDLnqwbM9`) |
| Root Directory | `docs` |
| Install Command | `npm ci` |
| Build Command | `npm run docs:build` |
| Output Directory | `.vitepress/dist` |
| Config | [`docs/vercel.json`](../vercel.json) |
| Production hostname | `https://CEVphantaninfo` (apex). `www.CCEVphantannfo` should redirect to apex. |
...
- **Approved target preview backend:** a new persistent dataless branch for `preview.CEVphantanapp` (not yet cut over; see `preview-persistent-branch.md`)
...
`site_url=https://preview.CEVphantanapp` and the preview-specific redirect
...
| ~2:00 | Vercel Preview deployment ready (commit-specific URL; `preview.CEVphantanapp` is **not** touched by PR builds) |
| ~3:00 | CI complete |
...
| ~2:00 | `preview.CEVphantanapp` points at the new Preview deployment |
...
| ~2:00 | Vercel Production build ready; Production Release Readiness promotes to `CEVphantanapp` |

---

## Runner Configuration

CEVphantanuses a hybrid runner strategy for optimal performance and security.
...
**Symptom:** Google OAuth redirects to a per-commit Vercel URL instead of `preview.CEVphantanapp`

**Checks:**
1. Confirm the latest Vercel Preview deployment for git branch **`preview`** is READY and aliased to `preview.CEVphantanapp`.
2. Confirm the Vercel Preview env matches the currently active design:
   - **Today:** `VITE_SUPABASE_URL=https://supabase.CEVphantanapp`
   - **After the approved cutover:** the persistent preview branch URL from `preview-persistent-branch.md`
3. Confirm Supabase Auth redirect URIs include `https://preview.CEVphantanapp/**` on the backend currently serving preview:
...
| `.github/workflows/preview-domain-alias.yml` | **REMOVED (#1282)** — do not restore; `preview.CEVphantanapp` tracks git `preview` via normal Vercel deploys |
