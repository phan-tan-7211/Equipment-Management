...
> Operational reference for the CEVphantanuptime monitors and public status page hosted by Better Stack.

## Overview

Better Stack provides two uptime monitors and a public status page for CEVphantan

| Component | Purpose |
|---|---|
| **Web Availability Monitor** | Proves the production SPA at `https://CEVphantanapp/` responds with HTTP 200 |
| **Deep Health Monitor** | Proves the Supabase backend and database are reachable via the healthcheck edge function |
| **Public Status Page** | Customer-facing uptime dashboard at `https://status.CEVphantanapp` |
...
| Field | Value |
|---|---|
| Monitor name | `CEVphantanWeb` |
| Target URL | `https://CEVphantanapp/` |
...
| Field | Value |
|---|---|
| Monitor name | `CEVphantanAPI Health` |
...
| Field | Value |
|---|---|
| Status page title | CEVphantanStatus |
| Status page URL | `https://status.CEVphantanapp` |
| Better Stack subdomain | `CEVphantanbetteruptime.com` |
| Components shown | CEVphantanWeb, CCEVphantanPI Health |
| History / uptime chart | Enabled (90-day history) |

## DNS / Custom Domain Setup

`status.CEVphantanapp` points to the Better Stack-hosted status page via a CNAME record.

### Steps

1. In the Better Stack status page settings, enable the custom domain and note the **CNAME target** provided.
2. In the **Vercel dashboard** (where `CEVphantanapp` DNS is managed):
   - Navigate to the domain `CEVphantanapp` > DNS Records.
   - Add a CNAME record:
     - **Name:** `status`
     - **Value:** `statuspage.betteruptime.com`
     - **TTL:** Auto / 300
3. Wait for DNS propagation (typically < 5 minutes on Vercel).
4. Verify: `https://status.CEVphantanapp` should load the Better Stack status page.
...
- If `CEVphantanapp` DNS moves away from Vercel, recreate the `status` CNAME at the new provider.
...
1. **Uptime monitors** — call `uptime_list_monitors_tool`. Pass: no auth/tool errors; expected monitors include **CEVphantanWeb** and **CCEVphantanPI Health** (see tables above).
