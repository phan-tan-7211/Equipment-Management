...
| Preview (cloud) | `https://supabase.CEVphantanapp/functions/v1/quickbooks-oauth-callback` | `https://supabase.CCEVphantanpp/functions/v1/google-workspace-oauth-callback` |
| Production | `https://supabase.CEVphantanapp/functions/v1/quickbooks-oauth-callback` | `https://supabase.CCEVphantanpp/functions/v1/google-workspace-oauth-callback` |
...
1. Set `PUBLIC_SITE_URL`:
   - Preview: `https://preview.CEVphantanapp`
   - Production: `https://CEVphantanapp`
2. **Remove** if present:
...
Update CEVphantanAgents vault items so future syncs do not restore retired knobs:
...
1. Open the CEVphantanapp → **Keys & OAuth**.
2. Ensure redirect URIs include the derived preview and production callback URLs above.
3. Remove any redirect URI using the retired host `supabase.preview.CEVphantanapp`.

## Google Cloud Console

1. Open the shared OAuth web client used by CEVphantan
...
3. Remove any redirect URI using `supabase.preview.CEVphantanapp`.

## Validation

1. Deploy to `preview` and wait for `preview.CEVphantanapp` to serve the new build.
