# CEVphantanDevelopment Lifecycle
...
- **Default QA URL:** commit-specific Vercel Preview (`*.vercel.app`) after you push a work branch; **`preview.CEVphantanapp`** after merge to git **`preview`**.
...
→ (later) /release or preview → main → Production Release Readiness → CEVphantanapp
```
...
- Open the deployment URL from the PR or Vercel dashboard (`CEVphantan<hash>-columbia-cloudworks-llc.vercel.app`).
- After merge to git **`preview`**, **`preview.CEVphantanapp`** updates via that branch’s Vercel deploy.
...
- Validated on the **Preview deployment URL**, local stack, and/or **`preview.CEVphantanapp`** (as appropriate).
