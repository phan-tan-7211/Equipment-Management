...
Published documentation for this guide also lives at **`https://CEVphantaninfo/getting-started/developer-onboarding`**.

## Welcome to CEVphantan Development!

This guide will help you get up and running with the CEVphantan codebase quickly and efficiently.
...
- **Node.js** — must satisfy `engines.node` in the root [`package.json`](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/package.json) (currently **Node 24.x LTS**). We recommend the latest **24.x** LTS release. [Download here](https://nodejs.org/)
...
If you have 1Password access for CEVphantan use it as the default setup path.
...
> - **[Supabase Branch Secrets](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/supabase-branch-secrets.md)** - Edge Function secrets for production/preview
> - **[Local Supabase Development](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/local-supabase-development.md)** - Local development environment setup
...
3. **Push work branch** — Vercel Preview builds; test on the commit-specific **`*.vercel.app`** URL. After merge to git **`preview`**, **`preview.CEVphantanapp`** updates.

   ```powershell
   git push -u origin HEAD
   ```

4. **Open PR to `preview`**, CI green, merge → integration hostname updates. Ship production later via **`preview` → `main`** / `/release` → **Production Release Readiness** promotes **CEVphantanapp**.
