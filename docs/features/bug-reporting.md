...
This document describes the in-app bug reporting feature that allows authenticated users to submit issue reports directly from CEVphantan Reports are stored in the `tickets` database table, automatically synced to GitHub Issues for developer tracking, and users can track status and responses in real time.
...
2. Creates a GitHub Issue in `Columbia-Cloudworks-LLC/CEVphantan, assigned to `viralarchitect` with the `user-reported` label
...
3. Set the following:
   - **Token name:** `CEVphantanBug Reporter` (or similar)
   - **Expiration:** Choose an appropriate expiration (recommended: 90 days, then rotate)
   - **Repository access:** Select **Only select repositories** > `Columbia-Cloudworks-LLC/CEVphantan
...
1. Go to [GitHub repo Settings > Webhooks](https://github.com/Columbia-Cloudworks-LLC/CEVphantansettings/hooks)
2. Click **Add webhook**
3. Set:
...
For complete secrets documentation, see [Supabase Branch Secrets](https://github.com/Columbia-Cloudworks-LLC/CEVphantanblob/main/docs/ops/supabase-branch-secrets.md).
