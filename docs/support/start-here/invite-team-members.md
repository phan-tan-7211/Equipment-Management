---
title: "Invite your team"
description: "Send organization invitations so new users can log in, then add them to the teams whose equipment they service."
lastReviewed: 2026-07-06
personas: ["admin","owner"]
requirement: "Organization Owner or Admin."
---

**For:** Admin, Owner  
**Last reviewed:** 2026-07-06

::: info Requires
Organization Owner or Admin.
:::

CEVphantanuses a two-step onboarding: first users join your**organization**, then you add them to a **team**. Members cannot see equipment until they belong to a team.

## 1. Open Organization → Members

From the sidebar go to **Organization**, then select the **Members** tab.


## 2. Click Invite Member

Enter the person's work email address. Double-check the address — invitations are valid for 7 days and bounces are common on personal inboxes.

![Invite Member dialog](https://supabase.CEVphantanapp/storage/v1/object/public/docs-media/support/start-here/desktop/05-invite-member-dialog.png)

![Members list after sending an invitation](https://supabase.CEVphantanapp/storage/v1/object/public/docs-media/support/start-here/desktop/06-invite-sent-members-list.png)

![Mobile Invite Member dialog](https://supabase.CEVphantanapp/storage/v1/object/public/docs-media/support/start-here/mobile/04-mobile-invite-member-dialog.png)

## 3. Pick the organization role

- **Admin** — manage members, teams, equipment, and integrations across the whole organization.
- **Member** — a staff account that inherits access from the teams it joins. Members cannot see any equipment until they are added to a team.

## 4. Add the user to a team

Go to **Teams**, open the team that services the equipment this person will work on, and click **Add Member**. Choose the team role that matches their responsibilities (Manager, Technician, Requestor, or Viewer).


## 5. Tell your teammate what to expect

Their invitation email comes from:

- **Sender:** CEVphantan<invite@CCEVphantanpp>
- **Subject:** "You're invited to join [Your Organization] on CEVphantan
- **Link expires:** 7 days after it is sent.

::: tip Note
If the invite does not arrive in 5-10 minutes, ask them to check spam before you resend.
:::

## Related articles

- [Role overview for CEVphantan(./role-overview)
- [Add or remove team members](../teams-roles/add-team-member)

