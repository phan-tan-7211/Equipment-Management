-- ============================================================================
-- Migration: fix_overdue_filter_canonical
--
-- Fixes: #665 (overdue sparkline undercounts historically-overdue completed WOs)
-- Relates to: 20260424000100, 20260425000000
--
-- Context
-- -------
-- 20260424000100 introduced the #665 overdue fix inside a DO $guard$ block that
-- skips when equipment_status_history already exists (assuming 20260425000000
-- already carried the fix).
--
-- However, 20260425000000 was modified IN-PLACE after its initial commit and
-- deployment to the Supabase preview branch. Environments that applied the
-- original version of 20260425000000 (before the in-place edits) still have
-- the old AND w.status NOT IN ('completed','cancelled') filter in place.
-- Combined with the guard-skip in 20260424000100, the fix was never applied
-- to those environments.
--
-- This migration closes that gap by unconditionally replacing
-- get_dashboard_trends with the correct overdue_events filter, selecting the
-- appropriate function body based on whether equipment_status_history exists.
--
-- Overdue filter semantics (#665)
-- --------------------------------
-- A work order is included in the overdue count on a given day if:
--   1. it has a due_date in the past (relative to that day), AND
--   2. it was not yet completed on that day.
-- The correct gate is:
--   (w.status NOT IN ('completed','cancelled') OR w.completed_day IS NOT NULL)
-- This includes completed/cancelled WOs only when they have a completed_date
-- that bounds the overdue period. WOs with status IN ('completed','cancelled')
-- AND completed_date IS NULL are excluded — COALESCE would otherwise default
-- to v_today, making them appear perpetually overdue.
-- ============================================================================
...
BEGIN
  IF EXISTS (
    ...
    -- equipment_status_history is present — use the point-in-time version from
    -- 20260425000000, guaranteed to carry the correct overdue filter.
    EXECUTE $fn$
  ...
  ELSE
    -- equipment_status_history not yet present — use the simpler version from
    -- 20260424000100, with the correct overdue filter.
    EXECUTE $fn$
    CREATE OR REPLACE FUNCTION public.get_dashboard_trends(
      p_org_id uuid,
      p_days integer DEFAULT 7
    ...
    BEGIN
      ...
      overdue_events AS (
        ...
        FROM (
          ...
          -- whichever is later). Completed/cancelled WOs are included when they have
          -- a completed_date: it bounds the overdue period correctly. WOs with
