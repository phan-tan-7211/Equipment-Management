...
-- queries that join cevphantan's auth.users + organizations against live Stripe
...
--   cevphantan has no live mapping today between organizations and Stripe
...
--   leak data through. When cevphantan reintroduces an org → Stripe mapping in a
...
BEGIN
  ...
  -- IMPORT FOREIGN SCHEMA — restricted to the 8 objects cevphantan may join
...
-- cevphantan introduces a real org → Stripe customer mapping (see migration
...
COMMENT ON FUNCTION public.list_active_stripe_subscriptions() IS
  ...
  'cevphantan reintroduces an org → Stripe customer mapping. Server-side Edge '
