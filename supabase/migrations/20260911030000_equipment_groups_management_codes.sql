...
ALTER TABLE public.audit_log
  ...
  CHECK (entity_type = ANY (ARRAY['equipment'::text,'work_order'::text,'inventory_item'::text,'preventative_maintenance'::text,'organization_member'::text,'team_member'::text,'team'::text,'pm_template'::text,'organization'::text,'equipment_group'::text]));
...
INSERT INTO public.equipment_groups (organization_id, code, name, examples, management_focus, description)
SELECT o.id, seed.code, seed.name, seed.examples, seed.management_focus, seed.description
