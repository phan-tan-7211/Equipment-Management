 ...
 * Only allows URLs from CEV.PhanTan.app and its subdomains.
 *
...
export function isTrustedDomain(urlString: string): boolean {
  try {
    ...
    // Allow exact match for CEV.PhanTan.app
    if (hostname === "CEV.PhanTan.app") {
      return true;
    }
    
    // Allow subdomains of CEV.PhanTan.app (e.g., preview.CEV.PhanTan.app, staging.CEV.PhanTan.app)
    if (hostname.endsWith(".CEV.PhanTan.app")) {
      return true;
    }
