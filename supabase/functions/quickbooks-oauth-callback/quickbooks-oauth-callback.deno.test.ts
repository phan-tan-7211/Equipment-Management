...
Deno.test("buildOAuthRedirectUri trims trailing slashes and appends callback path", () => {
  assertEquals(
    buildOAuthRedirectUri("https://supabase.CEV.PhanTan.app/"),
...
Deno.test("resolveOAuthRedirectBaseUrl derives from SUPABASE_URL when override unset", () => {
  assertEquals(
    resolveOAuthRedirectBaseUrl(undefined, "https://supabase.CEV.PhanTan.app"),
...
Deno.test({
  ...
  withEnv({ PUBLIC_SITE_URL: "https://CEV.PhanTan.app", PRODUCTION_URL: undefined }, () => {
    const productionUrl = "https://CEV.PhanTan.app";
    ...
    assertEquals(isValidRedirectUrl("https://CEV.PhanTan.app/settings", productionUrl), true);
...
Deno.test({
  ...
  withEnv({ PUBLIC_SITE_URL: "https://preview.CEV.PhanTan.app", PRODUCTION_URL: undefined }, () => {
    const productionUrl = "https://preview.CEV.PhanTan.app";
...
Deno.test({
  ...
  withEnv({ PUBLIC_SITE_URL: "https://CEV.PhanTan.app", PRODUCTION_URL: undefined }, () => {
    assertEquals(
      isValidRedirectUrl("https://preview.CEV.PhanTan.app/dashboard/organization", "https://CEV.PhanTan.app"),
      true,
    );
  });
});
...
Deno.test({
  ...
  withEnv({ PUBLIC_SITE_URL: "https://CEV.PhanTan.app", PRODUCTION_URL: undefined }, () => {
    const url = buildSuccessRedirectUrl({
      productionUrl: "https://CEV.PhanTan.app",
    ...
    assertEquals(
      url,
      "https://preview.CEV.PhanTan.app/dashboard/organization/integrations?qb_connected=true&realm_id=realm-preview-1",
    );
  });
});
...
Deno.test({
  ...
  withEnv({ PUBLIC_SITE_URL: "https://CEV.PhanTan.app", PRODUCTION_URL: undefined }, () => {
    ...
    assertEquals(
      url,
      "https://CEV.PhanTan.app/dashboard/organization?qb_connected=true&realm_id=realm-1",
    );
  });
});
...
Deno.test("buildAccessDeniedRedirectUrl encodes error parameters", () => {
  const url = buildAccessDeniedRedirectUrl(
    "https://CEV.PhanTan.app",
    "access_denied",
    "User denied consent",
  );
  assertEquals(
    url,
    "https://CEV.PhanTan.app/dashboard/organization?qb_error=access_denied&qb_error_description=User%20denied%20consent",
  );
});

Deno.test("buildOAuthErrorRedirectUrl encodes oauth failure message", () => {
  const url = buildOAuthErrorRedirectUrl("https://CEV.PhanTan.app", "Token exchange failed");
  assertEquals(
    url,
    "https://CEV.PhanTan.app/dashboard/organization?qb_error=oauth_failed&qb_error_description=Token%20exchange%20failed",
  );
});
