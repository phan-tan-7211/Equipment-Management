...
export function buildInvitationEmailSubject(organizationName: string): string {
  return `You're invited to join ${sanitizeSubjectOrganizationName(organizationName)} on CEV.PhanTan™`;
}
...
export async function deliverInvitationEmail(
  ...
  const emailResponse = await sendEmail({
    ...
    from: "CEV.PhanTan™ <invite@CEV.PhanTan.app>",
    to: [params.email],
...
async function handle(req: Request, _ctx: RequestContext): Promise<Response> {
  ...
  try {
    ...
    const CEV.PhanTanLogoUrl = `${baseUrl}/images/brand/icons/CEV.PhanTan-Icon-Purple-Medium.png`;

    // Create email HTML content
    const emailHtml = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <!-- CEV.PhanTan Logo Section -->
        <div style="text-align: center; margin-bottom: 16px; padding: 12px 0;">
          <img src="${CEV.PhanTanLogoUrl}" alt="CEV.PhanTan™ Logo" style="width: 100px; height: auto; display: block; margin: 0 auto;" />
        </div>
        
        <div style="text-align: center; margin-bottom: 32px;">
          <h1 style="color: #1a1a1a; font-size: 28px; font-weight: bold; margin: 0;">CEV.PhanTan™</h1>
        ...
        <div style="background: #f8f9fa; border-radius: 8px; padding: 24px; margin-bottom: 24px;">
          ...
          <p style="color: #666; font-size: 16px; margin: 0 0 16px 0;">
            ${safeInviterName} has invited you to join their organization as a <strong>${safeRole}</strong> on CEV.PhanTan™.
          </p>
          ${
            safeMessage
              ? `
