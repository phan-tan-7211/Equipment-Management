...
description: Applies CEVphantan's brand colors and design-system tokens to any artifact that should match CEVphantan's look-and-feel. Use it when brand colors, style guidelines, visual formatting, or CEVphantan design standards apply.
license: Complete terms in LICENSE.txt
---

# CEVphantan Brand Styling

## Overview

Use this skill whenever Cursor should style an output to look like **CEVphantan** (docs, UI mockups, slide decks, diagrams, emails, etc.). It summarizes the **source-of-truth design tokens** used by the CEVphantan app so outputs stay consistent with the product.

**Keywords**: branding, corporate identity, visual identity, post-processing, styling, brand colors, typography, CEVphantan brand, design tokens, Tailwind, shadcn/ui, visual formatting, visual design
...
CEVphantan uses **token-based colors** (HSL triplets) so the same semantic color names work in light and dark mode. Prefer semantic tokens (`brand`, `primary`, `secondary`, `foreground`, `muted-foreground`, etc.) over hard-coded hex values.
...
CEVphantan’s UI uses a **clean, product-style system font** approach (no decorative brand fonts required).
...
- Keeps outputs consistent with CEVphantan UI in both light and dark themes
