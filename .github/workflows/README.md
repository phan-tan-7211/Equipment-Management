...
|---|---|---|---|
| [`ci.yml`](./ci.yml) | TODO | `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`, `CODECOV_TOKEN` | `op://CEVphantan Agents/app-env-prod-public/SUPABASE_URL`, `op://CEVphantan Agents/app-env-prod-public/SUPABASE_ANON_KEY` (confirm field labels on the item with `op item get` metadata), `op://CEVphantan Agents/codecov-token/credential` |
| `preview-domain-alias.yml` | **REMOVED (#1282)** | — | Retired; see [`docs/ops/git-and-deploy.md`](../../docs/ops/git-and-deploy.md) — `preview.CEVphantan.app` tracks git `preview` via normal Vercel deploys |
| [`export-schema.yml`](./export-schema.yml) | TODO | `PREVIEW_DATABASE_URL` (→ production pooler after #1033) | `op://CEVphantan Agents/preview-database-url/credential` |
| [`production-release-readiness.yml`](./production-release-readiness.yml) | LIVE | `OP_SERVICE_ACCOUNT_TOKEN` + `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`, `VERCEL_TOKEN` via 1Password | `op://CEVphantan Agents/supabase-write/SUPABASE_ACCESS_TOKEN`, `op://CEVphantan Agents/supabase-write/prod_db_password`, `op://CEVphantan Agents/vercel-write/VERCEL_TOKEN` |
  ...
  build:
    ...
    steps:
      ...
      - uses: ./.github/actions/load-1p-secrets
        env:
          VITE_SUPABASE_URL:      op://CEVphantan Agents/app-env-prod-public/SUPABASE_URL
          VITE_SUPABASE_ANON_KEY: op://CEVphantan Agents/app-env-prod-public/SUPABASE_ANON_KEY
      - run: npm run build
...
Migrate after the corresponding 1Password items exist in the CEVphantan Agents
