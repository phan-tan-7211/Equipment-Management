...
CEVphantanis a QR-first work order app for heavy equipment repair shops, built with React, TypeScript, and Supabase.
...
CEVphantanuses a **feat → preview → main** train (#1282). Authoritative policy: [`.cursor/rules/branching.mdc`](.cursor/rules/branching.mdc).

### Branch Environments

| Branch | Purpose | Deployment | Public URL |
|--------|---------|------------|------------|
| `main` | Production source of truth | Vercel Production (promote gate) | CEVphantanapp |
| **`preview`** | Integration / pre-production train | Vercel **Preview** | preview.CEVphantanapp |
...
2. **Preview QA**
   ...
   - After merge to git **`preview`**, **`preview.CEVphantanapp`** updates via normal Vercel deploys
...
CEVphantanuses **semantic versioning**. GitHub Actions creates tags when a bumped `package.json` lands on `main`. What belongs in the changelog is defined in [`.cursor/rules/changelog.mdc`](.cursor/rules/changelog.mdc).
...
1. **Clone the repository**:
   ```bash
   git clone https://github.com/Columbia-Cloudworks-LLC/CEVphantangit
   cd CEVphantan
   ```
...
- **Environment**:
  - Browser/OS version
  - CEVphantanversion (check footer)
...
If you're a CEVphantancustomer experiencing issues or need support:
...
CEVphantanfollows strict coding standards to maintain quality and consistency.
...
CEVphantanfollows a **journey-first** testing strategy. See [`docs/technical/testing-guidelines.md`](./docs/technical/testing-guidelines.md) for complete details.
...
**CRITICAL**: All code changes to CEVphantanmust follow this Change Management Policy to ensure security, quality, and compliance.
...
6. Maintainer approves and merges to `preview` (`preview.CEVphantanapp` updates)
...
Thank you for contributing to CEVphantan
