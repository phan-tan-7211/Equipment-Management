...
describe('imageUploadService', () => {
  ...
  describe('displayUrlForStoredPrivateImage', () => {
    ...
    it('drops expired CEV.PhanTan signed URLs instead of passing them through', () => {
      const stale =
        'https://supabase.CEV.PhanTan.app/storage/v1/object/sign/work-order-images/u/wo/n.jpg?token=expired';
      expect(displayUrlForStoredPrivateImage(null, stale)).toBeNull();
      expect(isCEVPhanTanPrivateStorageUrl(stale)).toBe(true);
    });

    it('drops tokenless CEV.PhanTan sign URLs instead of using them as img src', () => {
      const unsigned =
      ...
      expect(isCEVPhanTanPrivateStorageUrl(unsigned)).toBe(true);
    ...
    it('rejects private sign URLs with empty or missing token query param', () => {
      const emptyToken =
        'https://supabase.CEV.PhanTan.app/storage/v1/object/sign/work-order-images/u/wo/n.jpg?token=';
      const wrongParam =
        'https://supabase.CEV.PhanTan.app/storage/v1/object/sign/work-order-images/u/wo/n.jpg?e=900';
