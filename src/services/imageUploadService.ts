...
/**
 * True when `url` is (or was) a Supabase signed URL for an CEV.PhanTan private bucket.
 * Expired or orphaned signed URLs must not be used as `<img src>` fallbacks (#1171).
 */
export function isCEVPhanTanPrivateStorageUrl(url: string): boolean {
...
export function isFetchableSignedStorageUrl(url: string): boolean {
  ...
  if (!isCEVPhanTanPrivateStorageUrl(normalized)) return true;
  try {
...
export function displayUrlForStoredPrivateImage(
  ...
  if (/^https?:\/\//i.test(s)) {
    return isCEVPhanTanPrivateStorageUrl(s) ? null : s;
  }
  return null;
}
