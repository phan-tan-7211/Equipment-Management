    ...
    publicFeatures: {
      ...
      googleWorkspace: {
        ...
        benefits: [
          [
            ...
            'Kết nối tên miền Workspace và đồng bộ người dùng từ danh bạ Google. Lấy danh sách người trong tổ chức để thêm họ làm thành viên ZNTEQR™ chỉ với vài thao tác.',
          ...
          [
            ...
            'Thành viên đăng nhập bằng tài khoản Google. Quyền quản trị được cấp sau khi xác thực danh tính Workspace. Không cần quản lý mật khẩu ZNTEQR™ riêng.',
            'Đăng nhập bằng Google', 'Danh tính Workspace', 'Không cần mật khẩu khác',
          ],
        ],
        steps: [
          ['Kết nối Google Workspace', 'Trong Cài đặt tổ chức → Tích hợp, kết nối Google Workspace. Cho phép ZNTEQR™ truy cập danh bạ của tổ chức để liên kết tên miền với tổ chức ZNTEQR™.'],
          ...
          ['Đăng nhập và truy cập', 'Người được mời đăng nhập bằng tài khoản Google. Sau khi xác thực, họ được thêm vào tổ chức và truy cập ZNTEQR™ theo vai trò, không cần gửi thư mời thủ công.'],
        ],
        showcases: [
        ...
        seo: [
          'Đăng nhập SSO và đồng bộ danh bạ Google Workspace cho ZNTEQR',
          ...
          'ZNTEQR có thay thế xác thực nhiều lớp của Google không?',
          'Không. Chính sách xác thực Google vẫn áp dụng. ZNTEQR tuân theo yêu cầu MFA của Workspace.',
          'Quản trị viên có thể giới hạn vai trò của người được nhập không?',
          'Khi nhập, quản trị viên gán vai trò trong tổ chức ZNTEQR cho người dùng Workspace trước khi lời mời có hiệu lực.',
          ...
          'Tích hợp Google Workspace kết nối danh bạ của bạn với ZNTEQR™ qua vài bước.',
        ],
      },
      customerCrm: {
        content: [
      ...
      mobileFirstDesign: {
        content: [
          ...
          'Mở ZNTEQR trên điện thoại', 'Tạo tài khoản miễn phí và xử lý công việc tiếp theo ngay trên điện thoại.',
          'Mở ZNTEQR trên điện thoại',
        ],
        benefits: [
        ...
        seo: [
          ...
          'Cỡ chữ và vùng chạm có tính đến WCAG không?', 'ZNTEQR tuân theo hướng dẫn độ tương phản cho giao diện tối và cung cấp vùng chạm lớn phù hợp khi đeo găng.',
          ...
          'Cách các quy trình ưu tiên di động hoạt động', 'ZNTEQR™ hoạt động trong xưởng, ngoài hiện trường hoặc tại bàn làm việc.',
        ],
      },
    },
  },
  ko: {
    publicFeatures: {
      ...
      googleWorkspace: {
        ...
        benefits: [
          ['Google 디렉터리 동기화', '접근 권한이 필요한 사용자 가져오기', 'Workspace 도메인을 연결하고 Google 디렉터리의 사용자를 동기화하세요. 조직 구성원을 불러와 몇 번의 클릭으로 ZNTEQR™ 구성원으로 추가할 수 있습니다.', 'Google 디렉터리에서 동기화', '구성원 변경 시 재동기화', '도메인별 접근 권한'],
          ...
          ['Google 로그인', '안전하고 친숙한 로그인', '구성원은 Google 계정으로 로그인합니다. Workspace 신원을 인증한 뒤 관리자 권한이 부여됩니다. 별도 ZNTEQR™ 비밀번호를 관리할 필요가 없습니다.', 'Google로 로그인', 'Workspace 신원', '추가 비밀번호 불필요'],
        ],
        steps: [
          ['Google Workspace 연결', '조직 설정 → 연동에서 Google Workspace를 연결하세요. ZNTEQR™의 조직 디렉터리 접근을 승인하면 도메인이 ZNTEQR™ 조직에 연결됩니다.'],
          ...
          ['로그인 및 접근', '초대된 사용자는 Google 계정으로 로그인합니다. 인증 후 역할에 따라 조직과 ZNTEQR™에 접근할 수 있습니다. 별도의 수동 초대 이메일은 필요하지 않습니다.'],
        ],
        showcases: [
          ...
          ['ZNTEQR에 가져온 Google Workspace 구성원과 역할을 보여주는 팀 명단', 'Google Workspace에서 가져오기', '동기화된 디렉터리에서 조직 구성원을 선택하고 역할을 지정한 뒤 확인하세요. 가져온 구성원은 명단에 표시되며 Google 계정으로 바로 로그인할 수 있습니다.'],
        ],
        seo: [
          'ZNTEQR을 위한 Google Workspace SSO 및 디렉터리 동기화',
          '기술자가 Google Workspace로 로그인하고 디렉터리를 동기화해 별도 비밀번호 없이 구성원을 등록할 수 있습니다.',
          'Google Workspace', 'ZNTEQR을 위한 Google Workspace SSO 및 디렉터리 동기화',
          '디렉터리에서 사용자를 가져오고 기존 Google 계정으로 로그인하세요.',
          'ZNTEQR이 Google MFA를 대체하나요?', '아니요. Google 인증 정책이 그대로 적용되며 ZNTEQR은 Workspace의 MFA 요구사항을 따릅니다.',
          '관리자가 가져올 사용자의 역할을 제한할 수 있나요?', '가져오기 과정에서 관리자가 Workspace 사용자의 ZNTEQR 조직 역할을 지정한 후 초대가 활성화됩니다.',
          '신규 입사자도 디렉터리 동기화에 반영되나요?', '재동기화하면 구성원 목록이 갱신되므로 CSV를 수동으로 처리하지 않아도 인사 변동을 반영할 수 있습니다.',
          'Google Workspace를 통한 구성원 등록 방법', 'Google Workspace 연동으로 몇 단계 만에 디렉터리와 ZNTEQR™을 연결할 수 있습니다.',
        ],
      },
      customerCrm: {
        content: [
      ...
      mobileFirstDesign: {
        content: [
          ...
          '휴대폰에서 ZNTEQR 열기', '무료 계정을 만들고 다음 작업을 휴대폰에서 처리하세요.', '휴대폰에서 ZNTEQR 열기',
        ],
        benefits: [
          ...
          ['현장에서는 휴대폰, 사무실에서는 컴퓨터', '모든 화면에서 같은 계정', 'iOS, Android 또는 컴퓨터에서 ZNTEQR을 사용하세요. 어디서나 같은 계정과 데이터가 제공됩니다. 기술자는 현장에서 휴대폰으로, 관리자는 큰 화면으로 작업합니다.', '모든 기기에서 사용', '반응형 화면', '모든 화면을 위한 하나의 앱'],
        ],
        steps: [
        ...
        seo: [
          ...
          '글꼴과 터치 영역이 WCAG를 고려하나요?', 'ZNTEQR은 다크 테마 대비 지침을 따르고 장갑을 낀 손에도 편리하도록 넉넉한 터치 영역을 제공합니다.',
          '태블릿에서도 데스크톱과 같은 지도를 사용할 수 있나요?', '장비 지도는 화면에 맞게 조정되어 현장 담당자도 배차 화면과 동일하게 확인할 수 있습니다.',
          '모바일 우선 작업 흐름 이용 방법', 'ZNTEQR™은 작업장, 현장 또는 사무실에서 사용할 수 있습니다.',
        ],
      },
    },
  },
