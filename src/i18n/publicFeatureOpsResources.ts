...
export const publicFeatureOpsResources = {
  ...
  vi: { publicFeatures: {
    qrCodeIntegration: {
      ...
      showcases: [
        ['Hộp thoại mã QR thiết bị ZNTEQR với mã có thể quét, URL thiết bị và lựa chọn tải xuống', 'Mã QR của thiết bị', 'Mỗi thiết bị có mã QR riêng. Kỹ thuật viên quét bằng camera điện thoại để mở thông tin, lịch sử bảo dưỡng và lệnh công việc đang hoạt động mà không cần tải ứng dụng.'],
        ['Danh sách thiết bị ZNTEQR với nút mã QR trên từng thẻ, nhóm phụ trách và ngày bảo dưỡng gần nhất', 'Truy cập nhanh từ danh sách thiết bị', 'Mỗi thẻ thiết bị có nút mã QR. Xem, tải xuống hoặc in mã ngay trong danh sách mà không cần mở từng trang chi tiết.'],
      ],
      seo: [
        ...
        'Quét mã QR thiết bị bằng điện thoại để xem lịch sử dịch vụ, lệnh công việc và danh sách bảo dưỡng định kỳ. Tạo nhãn QR để in cho đội máy.',
        'Kỹ thuật viên có cần cài ứng dụng để quét mã QR không?', 'Kỹ thuật viên có thể dùng ứng dụng web ZNTEQR trên trình duyệt điện thoại hoặc camera tích hợp tùy thiết bị. Nhãn QR chứa URL HTTPS dẫn thẳng đến luồng phù hợp sau khi đăng nhập.',
        'Quét QR có thể kết nối khách hàng với yêu cầu dịch vụ không?', 'ZNTEQR hỗ trợ các luồng dùng QR để người quét mở trải nghiệm phù hợp với vai trò, cấu hình tổ chức và nhóm được phân công.',
        'ZNTEQR có miễn phí cho xưởng sửa chữa máy hạng nặng không?', 'ZNTEQR có gói miễn phí dành cho xưởng sửa chữa, với số người dùng không giới hạn và hạn mức hợp lý như dung lượng lưu trữ. Có thể mở rộng khi nhu cầu tăng.',
        'Cách theo dõi bằng mã QR trong ZNTEQR', 'In nhãn, dán lên máy rồi quét khi làm việc.',
      ],
    },
    workOrderManagement: {
      content: [
      ...
      seo: [
        ...
        'ZNTEQR có hỗ trợ phân công theo nhóm không?', 'Có thể giao lệnh cho nhóm hoặc kỹ thuật viên riêng lẻ, kèm bước xác nhận để việc điều phối có người chịu trách nhiệm.',
        'Luồng xử lý lệnh công việc trong ZNTEQR', 'Lệnh công việc kết nối thiết bị, nhóm, mẫu bảo dưỡng định kỳ và kho vật tư trong một quy trình.',
      ],
    },
    inventoryManagement: {
      content: [
      ...
      seo: [
        ...
        'Cảnh báo tồn kho thấp có thông báo cho nhóm không?', 'ZNTEQR đánh dấu các ngưỡng tồn kho thấp trên bảng điều khiển để người mua hàng đặt lại trước khi công việc bị chậm.',
        'Kho vật tư có hỗ trợ mã vạch hoặc QR không?', 'ZNTEQR dùng nhãn QR để định danh cả thiết bị và kệ vật tư, giúp quét nhanh khi nhập và xuất.',
  ...
  ko: { publicFeatures: {
    qrCodeIntegration: {
      ...
      showcases: [
        ['스캔 가능한 QR 코드와 장비 URL 및 다운로드 옵션을 보여주는 ZNTEQR 장비 QR 코드 창', '장비 QR 코드', '장비마다 고유 QR 코드가 생성됩니다. 기술자가 스마트폰 카메라로 코드를 스캔하면 장비 정보, 정비 이력 및 진행 중인 작업 지시가 열립니다. 앱을 내려받을 필요가 없습니다.'],
        ['장비별 QR 버튼과 담당 팀 및 최근 정비일을 보여주는 ZNTEQR 장비 목록', '장비 목록에서 빠르게 접근', '모든 장비 카드에 QR 코드 버튼이 있습니다. 각각의 상세 페이지를 열지 않고도 목록에서 코드를 확인, 다운로드 또는 인쇄할 수 있습니다.'],
      ],
      seo: [
        ...
        '기술자가 QR 코드를 스캔하려면 앱을 설치해야 하나요?', '스마트폰 브라우저에서 ZNTEQR 웹 앱을 사용하거나 기기 설정에 따라 내장 카메라로 스캔할 수 있습니다. QR 스티커의 HTTPS URL은 로그인 후 해당 작업 화면으로 연결됩니다.',
        'QR 스캔으로 고객의 서비스 요청을 연결할 수 있나요?', 'ZNTEQR은 조직 설정과 팀 배정에 따라 QR을 스캔한 사용자를 역할에 맞는 화면으로 안내하는 워크플로를 지원합니다.',
        '중장비 정비소에서 ZNTEQR을 무료로 쓸 수 있나요?', 'ZNTEQR에는 정비소를 위한 무료 요금제가 있습니다. 사용자 수는 제한하지 않으며 저장 용량 등 합리적인 한도가 적용됩니다. 필요에 따라 용량을 확장할 수 있습니다.',
        'ZNTEQR에서 QR 코드 추적을 사용하는 방법', '라벨을 인쇄해 장비에 붙이고 작업 중 스캔하세요.',
      ],
    },
    workOrderManagement: {
      content: [
      ...
      seo: [
        ...
        'ZNTEQR에서 팀에 작업을 배정할 수 있나요?', '팀 또는 개별 기술자에게 작업을 배정하고 수락 절차를 거쳐 담당자를 명확히 할 수 있습니다.',
        'ZNTEQR의 작업 지시 처리 과정', '작업 지시는 장비, 팀, 예방 정비 템플릿 및 재고를 하나의 과정으로 연결합니다.',
      ],
    },
    inventoryManagement: {
      content: [
      ...
      seo: [
        ...
        '재고 부족 알림이 팀에 전달되나요?', 'ZNTEQR은 대시보드에서 재고 부족 기준을 강조해 구매 담당자가 작업 지연 전에 재주문할 수 있게 합니다.',
        '재고에 바코드 또는 QR을 사용할 수 있나요?', 'ZNTEQR은 장비와 재고 보관함 모두에 QR 라벨을 식별자로 사용해 입출고 시 스캔을 빠르게 합니다.',
