    ...
    publicFeatures: {
      fleetVisualization: {
        ...
        seo: [
          ...
          'ZNTEQR có cần thiết bị GPS không?', 'Không cần phần cứng GPS riêng. Xưởng có thể dùng địa chỉ hoặc tọa độ được ghi nhận khi điều phối.',
      ...
      quickBooks: {
        ...
        showcases: [
          ['Video di động mô tả xuất lệnh công việc ZNTEQR đã hoàn tất sang hóa đơn nháp QuickBooks Online', 'Xuất lệnh công việc sang QuickBooks trên điện thoại', 'Xem cách xuất lệnh công việc hoàn tất từ ZNTEQR sang hóa đơn nháp QuickBooks Online chỉ trong vài thao tác. Công lao động, phụ tùng và ánh xạ khách hàng được chuyển sang để bạn kiểm tra và gửi hóa đơn trong QuickBooks.'],
        ...
        seo: [
          ...
          'ZNTEQR tích hợp với sản phẩm QuickBooks nào?', 'ZNTEQR tích hợp QuickBooks Online qua Intuit OAuth. Trạng thái kết nối trong Cài đặt tổ chức giúp quản trị viên theo dõi token.',
          'Hóa đơn xuất cho khách hàng gồm những gì?', 'Bản xuất tổng hợp công lao động và phụ tùng theo quy tắc tính phí của ZNTEQR. Nhân viên hoàn thiện nội dung trong QuickBooks trước khi gửi.',
          'Có thể kết nối lại khi token QuickBooks hết hạn không?', 'Mở Cài đặt tổ chức rồi Tích hợp để kết nối lại. Các ánh xạ nhóm với khách hàng lưu trong ZNTEQR vẫn được giữ.',
    ...
    publicFeatures: {
      fleetVisualization: {
        ...
        seo: [
          ...
          'ZNTEQR에 GPS 장비가 필요한가요?', '별도의 GPS 하드웨어는 필요하지 않습니다. 작업 배치 시 이미 기록된 주소나 좌표를 사용할 수 있습니다.',
      ...
      quickBooks: {
        ...
        benefits: [
          ...
          ['조직 설정에서 연결', 'Intuit 로그인 후 고객 매핑', '조직 설정에서 OAuth로 QuickBooks를 연결하세요. Intuit 계정으로 로그인하고 ZNTEQR™을 승인하면 완료됩니다. 토큰이 자동 갱신되며 필요한 경우에만 다시 연결하면 됩니다.', '조직 설정에서 연결', '자동 토큰 갱신', '샌드박스 및 운영 환경'],
        ],
        steps: [
        ...
        showcases: [
          ['완료된 ZNTEQR 작업 지시를 모바일에서 QuickBooks Online 청구서 초안으로 내보내는 애니메이션', '모바일에서 작업 지시를 QuickBooks로 내보내기', '몇 번의 터치로 완료된 ZNTEQR 작업 지시가 QuickBooks Online 청구서 초안이 되는 과정을 확인하세요. 노무, 부품 및 고객 매핑이 전달되어 QuickBooks에서 검토하고 보낼 수 있습니다.'],
        ...
        seo: [
          ...
          'ZNTEQR은 어떤 QuickBooks 제품과 연동되나요?', 'ZNTEQR은 Intuit OAuth를 통해 QuickBooks Online과 연동됩니다. 관리자는 조직 설정에서 연결 및 토큰 상태를 확인할 수 있습니다.',
          '고객에게 보낼 청구서에는 어떤 내용이 포함되나요?', '내보낸 청구서에는 ZNTEQR 청구 규칙에 따른 노무와 부품 요약이 포함됩니다. 담당자가 QuickBooks에서 문구를 확정한 뒤 전송합니다.',
          'QuickBooks 토큰이 만료되면 다시 연결할 수 있나요?', '조직 설정의 연동 메뉴에서 다시 연결하세요. ZNTEQR에 저장된 팀과 고객의 매핑은 유지됩니다.',
