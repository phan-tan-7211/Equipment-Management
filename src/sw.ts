...
/**
 * CEV.PhanTan Service Worker (vite-plugin-pwa injectManifest source)
 *
...
self.addEventListener('push', (event) => {
  ...
  const options: NotificationOptions = {
    body: data.body,
    icon: '/images/brand/icons/CEV.PhanTan-Icon-Purple-Small.png',
    badge: '/images/brand/icons/CEV.PhanTan-Icon-Purple-Small.png',
    ...
    tag: data.data?.notification_id || 'CEV.PhanTan-notification',
  };

  event.waitUntil(self.registration.showNotification(data.title || 'CEV.PhanTan', options));
});
...
registerRoute(
  ...
  new NetworkFirst({
    cacheName: 'CEV.PhanTan-pages-v1',
...
registerRoute(
  ...
  new CacheFirst({
    cacheName: 'CEV.PhanTan-static-v1',
    plugins: [
      new ExpirationPlugin({
        maxEntries: 80,
...
registerRoute(
  ...
  new CacheFirst({
    cacheName: 'CEV.PhanTan-supabase-storage-v1',
    plugins: [
      new ExpirationPlugin({
        maxEntries: 200,
        maxAgeSeconds: 7 * 24 * 60 * 60,
...
setCatchHandler(async ({ request }) => {
  if (request.mode === 'navigate') {
    ...
    // CEV.PhanTan-pages-v1. matchPrecache() resolves the versioned precache key
