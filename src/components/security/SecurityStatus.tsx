...
export const SecurityStatus = () => {
  ...
  const getStatusBadge = (success: boolean, hasErrors: boolean) => {
    if (hasErrors) return <Badge variant="destructive">{t('settingsSecurity.failed')}</Badge>;
    if (success) return <Badge variant="default" className="bg-success">{t('settingsSecurity.passed')}</Badge>;
    return <Badge variant="secondary">{t('settingsSecurity.pending')}</Badge>;
  };
    ...
    return (
      <div className="rounded-lg border p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">{t('settingsSecurity.securityStatus')}</span>
        </div>
        <div className="flex items-center gap-2 mt-2">
          <RefreshCw className="h-4 w-4 animate-spin" />
          <span className="text-sm text-muted-foreground">{t('settingsSecurity.securityLoading')}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="rounded-lg border p-4 space-y-2">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t('settingsSecurity.securityRefresh')}
              >
                <RefreshCw className="h-3.5 w-3.5" />
              </Button>
            </TooltipTrigger>
            <TooltipContent>{t('settingsSecurity.securityRefresh')}</TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
      ...
      <div className="flex items-center justify-between min-h-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{t('settingsSecurity.organizationAccess')}</span>
        </div>
      ...
      <div className="flex items-center justify-between min-h-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{t('settingsSecurity.memberAccess')}</span>
        </div>
      ...
      <div className="flex items-center justify-between min-h-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">{t('settingsSecurity.teamAccess')}</span>
        </div>
      ...
      {testResult.hasErrors && testResult.errors.length > 0 && (
        <div className="p-3 bg-destructive/10 border border-destructive/30 rounded-md">
          <h4 className="text-sm font-medium text-destructive mb-2">{t('settingsSecurity.securityIssues')}</h4>
      ...
      {!testResult.hasErrors && testResult.canFetchOrganizations && (
        <div className="p-3 bg-success/10 border border-success/30 rounded-md">
          <p className="text-xs text-success">
            {t('settingsSecurity.securitySuccess')}
          </p>
        </div>
      )}
    </div>
