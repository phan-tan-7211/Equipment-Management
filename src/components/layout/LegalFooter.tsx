      ...
      <div className="container mx-auto px-4 py-2">
        <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 text-xs leading-tight text-muted-foreground">
          <p className="inline-flex min-w-0 flex-wrap items-center gap-x-1">
            <span className="whitespace-nowrap">© {currentYear} CEV™</span>
            <Link
              to="/releases"
              className={`${linkClassName} font-medium`}
              aria-label={`View release notes for CEV version ${APP_VERSION}`}
            >
              v{APP_VERSION}
            </Link>
          ...
          >
            ...
            <ExternalLink href="https://status.CEV.PhanTan.app" className={linkClassName} showIcon={false}>
              Status
            </ExternalLink>
          </nav>
        </div>
