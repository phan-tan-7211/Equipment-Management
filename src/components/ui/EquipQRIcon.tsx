...
interface CEVPhanTanIconProps extends React.SVGProps<SVGSVGElement> {
  /**
   * Accessible label. Defaults to "CEV.PhanTan".
...
/**
 * CEV.PhanTan brand icon â€” the QR scanner mark in brand purple (#7B3EE7).
 *
 ...
 *
 * Usage:
 *   <CEVPhanTanIcon className="h-8 w-8" />
 *   <CEVPhanTanIcon className="h-6 w-6" title="" aria-hidden />
 */
const CEVPhanTanIcon = React.forwardRef<SVGSVGElement, CEVPhanTanIconProps>(
  ({ title = 'CEV.PhanTan', className, ...props }, ref) => {
    // Generate a per-instance id so multiple CEVPhanTanIcon renders on the same
    ...
    const titleId = title ? `CEV.PhanTan-icon-title-${reactId}` : undefined;
    return (
      <svg
        ref={ref}
...
CEVPhanTanIcon.displayName = 'CEVPhanTanIcon';
export default CEVPhanTanIcon;
