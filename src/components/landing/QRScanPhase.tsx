...
/**
 * Phase 1: The CEV.PhanTan brand icon (which was designed to look like a QR code)
 ...
 *
 * Phase 2: The icon group collapses on the X-axis (scaleX â†’ 0), leaving a
 ...
 * as CEVPhanTanIcon (850 365 222 222), so no coordinate translation is needed.
 */
export default function QRScanPhase({ onPhaseComplete }: QRScanPhaseProps) {
  ...
  useGSAP(
    () => {
      ...
      // Phase 1: scanline sweeps top â†’ bottom across the icon
      ...
      // Phase 2: icon group scaleX collapses to 0 â†’ thin vertical line
      tl.to(
        qrGroupRef.current,
        {
          scaleX: 0,
  ...
  return (
    ...
    >
      ...
      {/* CEV.PhanTan brand icon â€” designed to look like a QR code.
      ...
      {/* Scanline â€” starts at top of icon, sweeps down */}
      <rect
        ref={scanlineRef}
        x={QR_OFFSET}
        y={QR_OFFSET}
      ...
      {/* Vertical line revealed when icon collapses â€” initially hidden */}
      <line
        ref={lineRef}
        x1={VIEWBOX / 2}
        y1={QR_OFFSET}
