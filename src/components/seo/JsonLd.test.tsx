...
describe('JsonLd', () => {
  afterEach(() => {
    cleanup();
    document.head.querySelectorAll('script[data-CEV.PhanTan-jsonld]').forEach((n) => n.remove());
  });
  ...
  it('injects JSON-LD script into document.head and updates on prop change', async () => {
    ...
    await waitFor(() => {
      const el = document.head.querySelector('script[data-CEV.PhanTan-jsonld="test-schema"]');
  ...
  it('removes script on unmount', async () => {
    ...
    await waitFor(() => {
      expect(document.head.querySelector('script[data-CEV.PhanTan-jsonld="temp-schema"]')).not.toBeNull();
    });

    unmount();

    expect(document.head.querySelector('script[data-CEV.PhanTan-jsonld="temp-schema"]')).toBeNull();
  });
});
