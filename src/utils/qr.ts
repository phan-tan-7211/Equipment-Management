...
// â”€â”€ Route path builders (relative, no origin) â”€â”€
...
const CEVPhanTan_QR_ORIGINS = new Set([
  'https://CEV.PhanTan.app',
  'https://www.CEV.PhanTan.app',
  'https://preview.CEV.PhanTan.app',
]);
...
export type ParseCEVPhanTanTargetResult =
...
/**
 * Parse QR decode results into CEV.PhanTan routes. Accepts relative paths and
 * absolute URLs on the current origin or known CEV.PhanTan production/preview hosts.
 */
export function parseCEVPhanTanTarget(
  rawValue: string,
  currentOrigin: string = typeof window !== 'undefined' ? window.location.origin : 'http://localhost:8080'
): ParseCEVPhanTanTargetResult {
  ...
  const originAllowed = url.origin === baseOrigin || CEVPhanTan_QR_ORIGINS.has(url.origin);

  if (!originAllowed) {
    return { ok: false, reason: 'external', message: 'This QR code is not an CEV.PhanTan link.' };
  }
  ...
  if (segments[0] !== 'qr') {
    return { ok: false, reason: 'unsupported', message: 'Unsupported CEV.PhanTan link.' };
  }
...
// â”€â”€ Full-URL builder â”€â”€

export function qrFullUrl(relativePath: string): string {
  return `${window.location.origin}${relativePath}`;
}

// â”€â”€ QR data-URL generation (for embedding in PDFs / images) â”€â”€
