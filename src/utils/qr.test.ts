...
describe('parseCEVPhanTanTarget', () => {
  it('parses relative equipment path with org', () => {
    const r = parseCEVPhanTanTarget('/qr/equipment/eq-1?org=org%20a', LOCAL_ORIGIN);
  ...
  it('parses absolute production equipment URL', () => {
    const r = parseCEVPhanTanTarget('https://CEV.PhanTan.app/qr/equipment/eq-99', LOCAL_ORIGIN);
    expect(r.ok && r.kind === 'equipment' && r.equipmentId === 'eq-99').toBe(true);
  });

  it('parses preview host equipment URL', () => {
    const r = parseCEVPhanTanTarget('https://preview.CEV.PhanTan.app/qr/equipment/eq-p', LOCAL_ORIGIN);
    expect(r.ok && r.kind === 'equipment' && r.equipmentId === 'eq-p').toBe(true);
  });

  it('parses legacy /qr/:equipmentId', () => {
    const r = parseCEVPhanTanTarget('/qr/legacy-id', LOCAL_ORIGIN);
  ...
  it('parses operator check-in token path', () => {
    const r = parseCEVPhanTanTarget('/qr/operator-check-in/token-abc', LOCAL_ORIGIN);
  ...
  it('parses quick form token path (#1184)', () => {
    const r = parseCEVPhanTanTarget('/qr/quick-form/token-qf', LOCAL_ORIGIN);
  ...
  it('encodes dynamic QR route segments before building redirect paths', () => {
    const equipment = parseCEVPhanTanTarget('/qr/equipment/eq%2Funsafe', LOCAL_ORIGIN);
    expect(equipment.ok && equipment.kind === 'equipment' && equipment.path).toBe(
      '/qr/equipment/eq%252Funsafe',
    );

    const inventory = parseCEVPhanTanTarget('/qr/inventory/inv%2Funsafe', LOCAL_ORIGIN);
    expect(inventory.ok && inventory.kind === 'inventory' && inventory.path).toBe(
      '/qr/inventory/inv%252Funsafe',
    );

    const workOrder = parseCEVPhanTanTarget('/qr/work-order/wo%2Funsafe', LOCAL_ORIGIN);
...
describe('qrFullUrl', () => {
  beforeEach(() => {
    Object.defineProperty(window, 'location', {
      value: { origin: 'https://CEV.PhanTan.app' },
      writable: true,
    });
  });

  it('prepends the current origin to a relative path', () => {
    expect(qrFullUrl('/qr/work-order/abc')).toBe('https://CEV.PhanTan.app/qr/work-order/abc');
  });
});
