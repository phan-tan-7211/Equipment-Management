...
describe('parseZNTEQRTarget', () => {
  it('parses relative equipment path with org', () => {
    const r = parseZNTEQRTarget('/qr/equipment/eq-1?org=org%20a', LOCAL_ORIGIN);
  ...
  it('parses legacy /qr/:equipmentId', () => {
    const r = parseZNTEQRTarget('/qr/legacy-id', LOCAL_ORIGIN);
  ...
  it('parses operator check-in token path', () => {
    const r = parseZNTEQRTarget('/qr/operator-check-in/token-abc', LOCAL_ORIGIN);
  ...
  it('parses quick form token path (#1184)', () => {
    const r = parseZNTEQRTarget('/qr/quick-form/token-qf', LOCAL_ORIGIN);
  ...
  it('encodes dynamic QR route segments before building redirect paths', () => {
    const equipment = parseZNTEQRTarget('/qr/equipment/eq%2Funsafe', LOCAL_ORIGIN);
    expect(equipment.ok && equipment.kind === 'equipment' && equipment.path).toBe(
      '/qr/equipment/eq%252Funsafe',
    );

    const inventory = parseZNTEQRTarget('/qr/inventory/inv%2Funsafe', LOCAL_ORIGIN);
    expect(inventory.ok && inventory.kind === 'inventory' && inventory.path).toBe(
      '/qr/inventory/inv%252Funsafe',
    );

    const workOrder = parseZNTEQRTarget('/qr/work-order/wo%2Funsafe', LOCAL_ORIGIN);
