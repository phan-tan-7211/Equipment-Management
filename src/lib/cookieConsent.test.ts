...
describe('cookieConsent', () => {
  ...
  it('returns null when no decision is stored', () => {
    ...
    localStorage.setItem('CEV.PhanTan:equipment-view-mode', 'table');
    expect(getPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode')).toBeNull();
  });
  ...
  it('persists Accept and allows preference storage', () => {
    ...
    expect(setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', 'table')).toBe(true);
    expect(localStorage.getItem('CEV.PhanTan:equipment-view-mode')).toBe('table');
  ...
  it('refuses preference writes for keys outside the optional allowlist', () => {
    ...
    expect(setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', 'grid')).toBe(true);
    expect(setPreferenceLocalStorage('CEV.PhanTan:work-orders-view-mode', 'calendar')).toBe(true);
  });

  it('returns false when preference setItem throws instead of crashing', () => {
    applyCookieConsentDecision('accepted');
    vi.spyOn(Storage.prototype, 'setItem').mockImplementation((key: string) => {
      if (key === 'CEV.PhanTan:equipment-view-mode') throw new Error('quota');
    });
    expect(setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', 'table')).toBe(false);
    vi.restoreAllMocks();
  });

  it('does not remove optional preference keys before Accept', () => {
    localStorage.setItem('CEV.PhanTan:selectedTeamId:org-1', 'team-A');
    removePreferenceLocalStorage('CEV.PhanTan:selectedTeamId:org-1');
    expect(localStorage.getItem('CEV.PhanTan:selectedTeamId:org-1')).toBe('team-A');
  ...
  it('persists Reject, blocks preference writes, and clears optional keys', () => {
    localStorage.setItem('CEV.PhanTan:equipment-view-mode', 'list');
    localStorage.setItem('CEV.PhanTan:selectedTeamId:org-1', 'team-A');
    localStorage.setItem('CEVPhanTan_offline_queue_u1_o1', '[]');
    ...
    expect(isPreferenceStorageAllowed()).toBe(false);
    expect(localStorage.getItem('CEV.PhanTan:equipment-view-mode')).toBeNull();
    expect(localStorage.getItem('CEV.PhanTan:selectedTeamId:org-1')).toBeNull();
    expect(localStorage.getItem('CEVPhanTan_offline_queue_u1_o1')).toBe('[]');
    expect(setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', 'table')).toBe(false);
    expect(localStorage.getItem('CEV.PhanTan:equipment-view-mode')).toBeNull();
  ...
  it('clearOptionalPreferenceStorage leaves consent and necessary keys alone', () => {
    ...
    localStorage.setItem('CEVPhanTan_pending_terms_acceptance:user-1', '1');
    localStorage.setItem('CEVPhanTan_session_data', '{"version":3}');
    ...
    expect(localStorage.getItem('CEVPhanTan_pending_terms_acceptance:user-1')).toBe('1');
    expect(localStorage.getItem('CEVPhanTan_session_data')).toBe('{"version":3}');
    expect(localStorage.getItem('CEVPhanTan_current_organization')).toBe('org-1');
    expect(localStorage.getItem('CEV.PhanTan:equipment-view-mode')).toBeNull();
  });
});
