...
describe('landingVideo', () => {
  ...
  it('uses VITE_SUPABASE_URL when it is already the production host', () => {
    vi.stubEnv('VITE_SUPABASE_URL', 'https://supabase.CEV.PhanTan.app/');
    expect(landingVideo('demo.mp4')).toBe(
      expectedVideoUrl('demo.mp4', 'https://supabase.CEV.PhanTan.app/'),
    );
  });

  it('ignores lookalike hosts and extra path segments on VITE_SUPABASE_URL', () => {
    vi.stubEnv('VITE_SUPABASE_URL', 'https://supabase.CEV.PhanTan.app.evil.com/storage/v1');
    expect(landingVideo('demo.mp4')).toBe(
      expectedVideoUrl('demo.mp4', 'https://supabase.CEV.PhanTan.app.evil.com/storage/v1'),
    );
  });
});
