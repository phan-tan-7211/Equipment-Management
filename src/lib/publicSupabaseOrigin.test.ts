...
const PUBLIC_ORIGIN = 'https://supabase.CEV.PhanTan.app';

describe('resolvePublicSupabaseOrigin', () => {
  it('uses VITE_SUPABASE_URL when it is already the production origin', () => {
    expect(resolvePublicSupabaseOrigin('https://supabase.CEV.PhanTan.app/')).toBe(PUBLIC_ORIGIN);
  });
  ...
  it('ignores lookalike hosts and extra path segments', () => {
    expect(
      resolvePublicSupabaseOrigin('https://supabase.CEV.PhanTan.app.evil.com/storage/v1'),
    ).toBe(PUBLIC_ORIGIN);
  });
