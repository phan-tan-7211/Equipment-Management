...
export const SimpleOrganizationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const {
    ...
    queryFn: async (): Promise<SimpleOrganization[]> => {
      ...
      // Fallback: session context not yet resolved â€” do the full fetch so the
  ...
  // Synchronization monitoring and recovery â€” session org is authoritative when it diverges
  // from CEVPhanTan_current_organization (e.g. QR redirect switched session without updating this key).
