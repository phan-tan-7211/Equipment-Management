    ...
    {isLoading ? <div className="space-y-3">{[...Array(3)].map((_,i)=><div key={i} className="h-12 bg-muted animate-pulse rounded-lg" />)}</div> : jitteredData.length>0 ? <div className="h-96 relative" ref={containerRef} role="img" aria-label={t('dashboardWidget.fleetEfficiencyAria',{count:jitteredData.length})}>
      <ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{top:10,right:20,bottom:10,left:0}}><CartesianGrid stroke="hsl(var(--border))" strokeDasharray="3 3" />
        ...
        <Scatter
          ...
          // recharts' ScatterShapeProps/AdaptChildMouseEventHandler types aren't
          // publicly exported, so these are cast via ComponentProps instead of
          // naming them directly.
          shape={((props:Record<string,unknown>)=><ScatterPointShape {...props} allPoints={jitteredData} onTeamSelect={handleTeamSelectFromCluster}/>) as unknown as React.ComponentProps<typeof Scatter>['shape']}
          onClick={(((point:unknown,_index:number,event:unknown)=>handlePointClick(point as JitteredPoint,event)) as unknown) as React.ComponentProps<typeof Scatter>['onClick']}
        />
