...
const variantStyles = { default:{border:'border-l-primary',text:'text-primary',chartColor:'hsl(var(--primary))'}, warning:{border:'border-l-warning',text:'text-warning',chartColor:'hsl(var(--warning))'}, danger:{border:'border-l-destructive',text:'text-destructive',chartColor:'hsl(var(--destructive))'} };
...
export const StatsCard: React.FC<StatsCardProps> = ({ icon,label,value,sublabel,to,trend,trendNote,sparkline,variant='default',loading=false,ariaDescription }) => {
  ...
  const content = <Card className={cn("border-l-[3px] transition-all duration-200",styles.border,variant==='warning'&&'bg-warning/5 dark:bg-warning/10',variant==='danger'&&'bg-destructive/5 dark:bg-destructive/10',to&&"cursor-pointer hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98]")} aria-label={ariaDescription}>
    <CardContent className={cn("p-4 pt-4 sm:p-5 sm:pt-5",hasSparkline?"pb-3 sm:pb-3":"pb-5 sm:pb-6")}>
      {loading ? <div className="space-y-3"><div className="flex items-center gap-2"><Skeleton className="h-4 w-4 rounded"/><Skeleton className="h-3 w-24"/></div><Skeleton className="h-9 w-16"/><Skeleton className="h-3 w-20"/></div> : <>
        <div className="flex items-center gap-2 mb-2"><span className={cn("flex-shrink-0",styles.text)}>{icon}</span><span className="text-[13px] font-medium text-muted-foreground tracking-wide truncate">{label}</span></div>
        <div className="text-3xl font-bold tracking-tight text-foreground" data-testid={`${label.toLowerCase().replace(/\s+/g,'-')}-value`}>{displayValue}</div>
