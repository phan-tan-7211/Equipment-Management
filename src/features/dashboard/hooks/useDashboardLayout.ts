...
function storageKey(userId: string, orgId: string): string {
  return `CEVPhanTan_dashboard_layout_${userId}_${orgId}`;
}
...
function readFromLocalStorage(userId: string, orgId: string): StoredPreferences | null {
  try {
    ...
    // Accept both new and legacy shapes â€” only activeWidgets is required.
 ...
 *
 * Layout is now purely an ordered list of widget IDs â€” no per-breakpoint
