...
type ColumnPointerDrag = {
  sourceColumnId: EquipmentTableColumnKey;
  pointerId: number;
  startX: number;
  startY: number;
  targetColumnId: EquipmentTableColumnKey;
  active: boolean;
...
const EquipmentTable: React.FC<EquipmentTableProps> = ({ equipment, onShowQRCode, sortConfig, onSortChange, visibleColumns, onToggleColumn, organizationId, columnFilterOptions, columnFilters, onColumnFilterChange }) => {
  ...
  const commitColumnReorder=useCallback((sourceColumnId:EquipmentTableColumnKey,targetColumnId:EquipmentTableColumnKey)=>{ if(sourceColumnId===targetColumnId)return; const nextVisibleOrder=[...orderedVisibleColumnKeys]; const sourceIndex=nextVisibleOrder.indexOf(sourceColumnId); const targetIndex=nextVisibleOrder.indexOf(targetColumnId); if(sourceIndex===-1||targetIndex===-1)return; nextVisibleOrder.splice(sourceIndex,1); nextVisibleOrder.splice(targetIndex,0,sourceColumnId); const pinned=nextVisibleOrder.filter((key)=>pinnedColumns.includes(key)); const unpinned=nextVisibleOrder.filter((key)=>!pinnedColumns.includes(key)); const nextVisible=[...pinned,...unpinned]; setColumnOrder((current)=>[...nextVisible,...current.filter((key)=>!nextVisible.includes(key))]); },[orderedVisibleColumnKeys,pinnedColumns,setColumnOrder]);
  ...
  const handleColumnDrop=useCallback((event:React.DragEvent<HTMLTableCellElement>,targetColumnId:EquipmentTableColumnKey)=>{ event.preventDefault(); event.stopPropagation(); const sourceColumnId=draggedColumnId??event.dataTransfer.getData('text/plain') as EquipmentTableColumnKey; if(sourceColumnId)commitColumnReorder(sourceColumnId,targetColumnId); columnInteractionRef.current=false; setDraggedColumnId(null); },[commitColumnReorder,draggedColumnId]);
