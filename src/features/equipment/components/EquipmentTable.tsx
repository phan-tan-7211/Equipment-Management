...
const EquipmentTable: React.FC<EquipmentTableProps> = ({ equipment, onShowQRCode, sortConfig, onSortChange, visibleColumns }) => {
  ...
  const columns=useMemo<ColumnDef<EquipmentTableRow>[]>(()=>{
    const dataColumns=visibleColumnKeys.map((columnKey)=>{ const meta=getEquipmentTableColumnMeta(columnKey); if(!meta)throw new Error(`Missing equipment table column meta for ${columnKey}`); return { ...createResizableSortableColumnBase(columnKey,columnSizing,meta,{active:sortConfig?.field===meta.sortField,sortOrder:sortConfig?.field===meta.sortField?sortConfig.direction:undefined,onSort:()=>handleSortClick(meta.sortField),hideVisibleTitle:columnKey===STATUS_COLUMN_KEY}), cell:({row})=>{ const item=row.original; switch(columnKey){
      case 'name': { const active=activeEquipmentId===item.id; return <button type="button" className="block w-full truncate text-left font-medium hover:text-primary" data-equipment-id={item.id} {...(active?{'data-equipment-transition-active':''}:{})} style={getEquipmentViewTransitionStyle('name',active)} onClick={()=>{void beginTransition({equipmentId:item.id,to:`/dashboard/equipment/${item.id}`});}}>{item.name}</button>; }
      case 'status': return <div className="flex items-center justify-center"><DotStatus status={item.status} className="mt-0" /></div>;
