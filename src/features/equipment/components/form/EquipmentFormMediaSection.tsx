    ...
    <Card>
      <CardContent className="space-y-3 pt-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" multiple className="hidden" onChange={(e) => { addFiles(Array.from(e.target.files ?? [])); e.target.value = ''; }} />
          <input ref={cameraInputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { addFiles(Array.from(e.target.files ?? [])); e.target.value = ''; }} />
        </div>
        ...
        {files.length > 0 ? (
          <div className="grid grid-cols-3 gap-2 sm:grid-cols-5">
            {files.map((file, index) => {
              ...
              return (
                <div key={`${file.name}-${file.size}-${index}`} className={cn('group relative aspect-square overflow-hidden rounded-md border bg-muted', isDisplay && 'ring-2 ring-primary')}>
