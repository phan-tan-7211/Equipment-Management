...
describe('EquipmentScanner', () => {
  beforeEach(() => {
    ...
    hoisted.MockQrScanner.scanImage.mockResolvedValue({
      data: 'https://CEV.PhanTan.app/qr/equipment/eq-upload',
    });
  });
  ...
  it('shows parser message for external URLs', async () => {
    ...
    await waitFor(() => expect(hoisted.scannerState.lastOnDecode).not.toBeNull());
    act(() => {
      hoisted.scannerState.lastOnDecode?.({ data: 'https://evil.example/not-CEV.PhanTan' });
    });
    const errorAlert = await screen.findByRole('alert');
    expect(errorAlert).toHaveTextContent(/not an CEV.PhanTan link/i);
