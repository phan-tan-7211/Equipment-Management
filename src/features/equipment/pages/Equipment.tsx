...
function readEquipmentViewMode(isMobile: boolean): EquipmentViewMode {
  const stored = getPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode');
...
const Equipment = () => {
  ...
  const rehydrateOrFlushViewMode = useCallback(() => {
    ...
    setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', viewModeRef.current);
  }, [isMobile]);
  ...
  const handleViewModeChange = useCallback((mode: EquipmentViewMode) => {
    setViewMode(mode);
    setPreferenceLocalStorage('CEV.PhanTan:equipment-view-mode', mode);
  }, []);
