...
import { parseCEVPhanTanTarget } from '@/utils/qr';
...
const EquipmentScanner: React.FC = () => {
  ...
  const handleDecodedPayload = useCallback(
    (raw: string, source: DecodeSource) => {
      ...
      const parsed = parseCEVPhanTanTarget(raw);
      if (!parsed.ok) {
      ...
      <Card>
        <CardHeader className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Point the camera at an CEV.PhanTan sticker, or upload a photo of the code.
          </p>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          {phase === 'decoded' && (
            <Alert>
              <AlertDescription data-testid="scanner-decoded-state">Opening scanned linkâ€¦</AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex flex-wrap gap-2 [&>button]:min-h-[44px]">
            ...
            >
              ...
              {isImageScanning ? 'Reading imageâ€¦' : 'Upload QR image'}
            </Button>
            <input
              ref={fileInputRef}
              type="file"
