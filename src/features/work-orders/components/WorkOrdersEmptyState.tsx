  ...
  return (
    <Card>
      <CardContent className="text-center py-12">
        ...
        <h3 className="text-lg font-semibold mb-2">{t('workOrders.list.noWorkOrdersFound')}</h3>
        <p className="text-muted-foreground mb-4">
          {hasActiveFilters
            ? isMyWorkActive
              ? t('workOrders.list.noWorkAssigned')
              : t('workOrders.list.noFilterMatches')
            : t('workOrders.list.createFirst')}
        </p>
        {!hasActiveFilters && (
          <Button onClick={onCreateClick}>
            <Plus className="h-4 w-4 mr-2" />
            {t('workOrders.list.createWorkOrder')}
          </Button>
        )}
      </CardContent>
    </Card>
