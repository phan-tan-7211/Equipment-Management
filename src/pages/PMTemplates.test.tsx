...
describe('PMTemplates Page', () => {
  ...
  function expandCEVPhanTanTemplates() {
    const trigger = screen.getByRole('button', { name: /CEV.PhanTan Templates/i });
  ...
  describe('Template Display', () => {
    it('renders global templates section', () => {
      ...
      expect(screen.getByText('CEV.PhanTan Templates')).toBeInTheDocument();
      expandCEVPhanTanTemplates();
    ...
    it('collapses CEV.PhanTan templates by default when the org already has a template', () => {
      render(
        <TestProviders>
          <PMTemplates />
        </TestProviders>
      ...
      expect(screen.getByRole('button', { name: /CEV.PhanTan Templates/i })).toHaveAttribute(
        'aria-expanded',
        'false',
      );
    ...
    it('expands CEV.PhanTan templates by default when the org has no templates of its own', () => {
      vi.mocked(usePMTemplates).mockReturnValue({
        ...mockHooks.usePMTemplates,
    ...
    it('toggles CEV.PhanTan and organization template sections from their headers', () => {
      render(
        <TestProviders>
          <PMTemplates />
        </TestProviders>
      ...
      fireEvent.click(screen.getByRole('button', { name: /CEV.PhanTan Templates/i }));
      expect(screen.getByText(pmFixtures.forklift.name)).toBeInTheDocument();
    ...
    it('reveals matching CEV.PhanTan templates when searching a collapsed section', () => {
      render(
        <TestProviders>
          <PMTemplates />
        </TestProviders>
    ...
    it('displays template card with correct data', () => {
      ...
      expect(
        within(forkliftCard).getByText(
          `${pmFixtures.forklift.sections.length} sections Â· ${pmFixtures.forklift.itemCount} items`,
        ),
      ).toBeInTheDocument();
      expect(within(forkliftCard).getByText('CEV.PhanTan')).toBeInTheDocument();
      expect(within(forkliftCard).getByText('Protected')).toBeInTheDocument();
    });

    it('keeps protected CEV.PhanTan starter titles readable when badges are present', () => {
      render(
        <TestProviders>
          <PMTemplates />
        </TestProviders>
      ...
      for (const templateName of ['Forklift PM Checklist', 'Compact Excavator PM', 'Excavator PM']) {
        ...
        expect(badgeRow).toHaveTextContent('CEV.PhanTan');
  ...
  describe('Template Actions', () => {
    it('renders the equipment assignment menu on every template card', () => {
      ...
      expect(screen.getAllByText('Ready to use â€” assign directly, no clone needed')).toHaveLength(3);
    });
