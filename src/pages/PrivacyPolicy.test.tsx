...
describe('PrivacyPolicy', () => {
  ...
  it('renders shell, navigation, numbered sections, and data-handling promises', () => {
    ...
    expect(screen.getByText(/When you create an account or interact with CEV.PhanTan/)).toBeInTheDocument();
    expect(screen.getByText(/3\. Information We Collect.*Organization Level/)).toBeInTheDocument();
    expect(screen.getByText(/Organizations that use CEV.PhanTan store business data/)).toBeInTheDocument();
  ...
  it('lists subprocessors, California rights tables, and contact / privacy-request links', () => {
    ...
    const phanTanLinks = screen.getAllByText('Phan Tan');
    phanTanLinks.forEach((link) => {
      expect(link.closest('a')).toHaveAttribute('href', 'https://phantan.com');
      expect(link.closest('a')).toHaveAttribute('target', '_blank');
      expect(link.closest('a')).toHaveAttribute('rel', 'noopener noreferrer');
    });
    expect(screen.getByText(/phantan7211@gmail\.com/)).toBeInTheDocument();
    expect(screen.getAllByText('CEV.PhanTan.app').length).toBeGreaterThanOrEqual(1);
    ...
    const privacyRequestLink = screen.getByText('CEV.PhanTan.app/privacy-request');
