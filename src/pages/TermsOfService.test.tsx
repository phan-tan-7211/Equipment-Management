...
describe('TermsOfService', () => {
  ...
  it('renders shell, core contractual sections, and contact links', () => {
    ...
    const links = screen.queryAllByText('Phan Tan');
    links.forEach((link) => {
      const anchor = link.closest('a');
      if (anchor) {
        expect(anchor).toHaveAttribute('href', 'https://phantan.com');
      }
    });
    expect(screen.getByText(/phantan7211@gmail\.com/i)).toBeInTheDocument();
    expect(screen.getByText(/https:\/\/phantan\.com/i)).toBeInTheDocument();
  });
});
