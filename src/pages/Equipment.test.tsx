...
describe('Equipment page', () => {
  beforeEach(() => {
    ...
    window.localStorage.removeItem('CEV.PhanTan:equipment-view-mode');
  });
  ...
  describe('viewMode localStorage hydration', () => {
    ...
    it('migrates legacy "list" persistence to card view', () => {
      window.localStorage.setItem('CEV.PhanTan:equipment-view-mode', 'list');
      setupHookForView();
      render(<Equipment />);
      expect(screen.getByTestId('grid-view-mode')).toHaveTextContent('grid');
    });
    ...
    it('hydrates "table" from localStorage', () => {
      window.localStorage.setItem('CEV.PhanTan:equipment-view-mode', 'table');
      setupHookForView();
      render(<Equipment />);
    ...
    it('falls back to "grid" for an unknown persisted value', () => {
      window.localStorage.setItem('CEV.PhanTan:equipment-view-mode', 'garbage');
      setupHookForView();
      render(<Equipment />);
      expect(screen.getByTestId('grid-view-mode')).toHaveTextContent('grid');
    });
