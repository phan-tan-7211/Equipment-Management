# CEVphantan� Cursor Agent Handbook
...
All agent-facing credentials live in the **CEVphantanAgents** 1Password vault. Do not invent parallel secret stores, commit `.env` files, or paste secret values into chat, commits, or PR bodies.
...
| Variable | Scope | Use |
| --- | --- | --- |
| `OP_SERVICE_ACCOUNT_TOKEN` (User scope) | Read-only service account `op-svc-CEVphantanagents` | `op read`, `op item list`, `op inject`, CI headless reads |
| `OP_SAT_CEVphantan (User scope) | **Write** service account on CCEVphantangents vault | `op item create`, `op item edit`, vault mutations only |
...
$app = (op read "op://CEVphantanAgents/app-env-preview-public/GOOGLE_WORKSPACE_CLIENT_ID").Trim()
$edge = (op read "op://CEVphantanAgents/edge-env-prod-secrets/GOOGLE_WORKSPACE_CLIENT_ID").Trim()
"appEdgeAligned=$($app -eq $edge)"
```

After changing `app-env-preview-public`, run `.\dev\sync-vercel-from-1password.ps1 -Environment preview` and confirm `preview.CEVphantanapp` is bound to git branch **`preview`** in Vercel (normal Preview deploys on merges/pushes to that branch � `preview-domain-alias.yml` is removed).
...
- `op read` / `op item get` with read SAT (CEVphantanAgents **and** Columbia Cloudworks Agents vaults)
...
- `OP_SAT_CEVphantan for 1Password vault writes (via `op-item-mutate.ps1` only)
...
```powershell
$env:OP_SERVICE_ACCOUNT_TOKEN = [Environment]::GetEnvironmentVariable('OP_SERVICE_ACCOUNT_TOKEN','User')
$env:OP_SAT_CEVphantan= [Environment]::GetEnvironmentVariable('OP_SAT_CCEVphantan'User')
```
...
| Google Workspace connect/disconnect on **preview cloud** only | "Please complete **Connect Google Workspace** on preview.CEVphantanapp; I'll watch edge logs / DB row count." Local GW flows are agent-automated via browser MCP � do not ask the user to click through local OAuth. |
