...
All notable changes to CEVphantanby Phan Tan will be documented in this file.
...
- **PM template groups (#1536)** — CEVphantanand organization sections on PM Templates can collapse. CCEVphantantarts closed when the organization already has a custom template.
...
- **Public releases page (#1460)** — The Legal footer version link now opens a public `/releases` page with build-time release notes from the CEVphantanchangelog.
...
- **Starter PM template titles stay readable** — CEVphantanstarter cards now keep template names readable even when both CCEVphantannd Protected badges are present.
...
- **Right to Repair stance** — Public `/right-to-repair` (Legal footer) states CEVphantansupports independent repair and will not hold customer records hostage. The page includes a filterable case atlas. It is a stance, not a contract.
...
- **CEVphantaninfo branding aligned with CCEVphantanpp (#1358)** — VitePress theme mirrors Mission Control HSL tokens (primary, neutrals, semantic status colors), uses `appearance: force-dark` (no light toggle), CECEVphantanrdmark with secondary Docs label, styles the Open App nav CTA as a primary button (system fonts; no third-party font CSS), honors `prefers-reduced-motion`, and documents token propagation in `docs/ops/deployment.md`.
...
- **E2E auth seeds cookie consent Accept** — Persona `storageState` from auth setup now persists `CEVphantancookie-consent=accepted` (and dismisses a visible banner) so the cookie consent strip cannot intercept Create Equipment, bottom nav, or FAB clicks.
...
- **Session hydrate re-syncs team memberships** — Cache load aligns `currentOrganizationId` with org preference and re-fetches `get_user_team_memberships` before clearing session loading, so a pinned/changed org cannot leave `teamMemberships: []` and empty the equipment list. E2E `pinContextToOrg` drops `CEVphantansession_data` so the preferred org reloads cleanly.
...
- **Google avatars in TopBar user menu (#1378)** — Resolve current-user avatar as CEVphantan`profiles.avatar_url` first, then Google Auth metadata (`avatar_url` / `picture`); wire `UserProfileMenu` trigger and menu header through `useResolvedAvatarUrl` with initials fallback. Deleting an CCEVphantanpload restores the Google photo when available.
...
- **Microphone voice input (#1170)** — Voice dictation buttons no longer fail: the `Permissions-Policy` header now allows microphone use on CEVphantans own pages (`microphone=(self)` in Vite dev, Vercel, and Netlify configs), and clicking a mic button explicitly requests browser microphone consent before starting speech recognition. If the user denies access, clicking the button again restarts the consent process with an actionable error message. Privacy policy copy updated to describe the browser-only dictation flow (audio is never recorded or stored by CCEVphantan
...
- **QuickBooks help-center guides** — Rewrote the Connect QuickBooks, Map Teams to QuickBooks Customers, and Export Work Orders to QuickBooks guides on CEVphantaninfo around a complete PM-work-order-to-invoice walkthrough, with real screenshots and demo videos hosted in the public `docs-media` bucket.
...
- **CEVphantaninfo Help Center interactivity and branding (#1147)** — The docs site CSP (`script-src 'self'`) blocked the inline scripts VitePress needs to hydrate, leaving the front page with dead feature cards, a dead hero button, and a non-functional theme toggle. Post-build CSP hash generation (`scripts/docs/generate-docs-csp.mjs`) now allowlists the three VitePress inline bootstraps without `script-src 'unsafe-inline'`. The Help Center also gains CCEVphantanranding: navbar logo, homepage hero logo, and favicon (previously 404). Playwright evidence serves the built docs through the exact production CSP to guard hydration, navigation, and theming.
...
- **Production release automation** — Production Release Readiness now runs `vercel promote` after migrations and schema drift pass, so CEVphantanapp updates without a manual Vercel dashboard promote.
- **Git and deploy workflow** — Development branches off `main` only; git branch **`preview`** kept as a Vercel domain anchor for optional **`preview.CEVphantanapp`**; default QA is each push’s **`*.vercel.app`** Preview URL.
...
- **Preview Google Workspace and QuickBooks connect** — Connect and reconnect on preview.CEVphantanapp now return to Organization Integrations after OAuth instead of failing when the callback lands with query parameters.

### Changed

- **Preview environment** — Preview at preview.CEVphantanapp now shares the production data backend used by CCEVphantanpp, so pull-request previews validate against real integration callbacks without maintaining a separate preview database.
...
- **CEVphantaninfo Help Center loads** — Returning visitors who still had the old CCEVphantanWA service worker registered now receive a kill-switch worker that clears stranded caches and unregisters so CECEVphantanfo serves the VitePress documentation site instead of the cached app shell.

## [3.8.5] - 2026-06-09

### Fixed

- **Help Center docs build** — Removed a broken internal link that blocked the CEVphantaninfo VitePress deployment on Vercel.
...
- **Internal ITIL workflow guidance** — Simplified agent-facing incident, problem, service request, change record, and issue resolver guidance so routine CEVphantanwork can move through lightweight triage and focused implementation without unnecessary process ceremony.
...
- **Sign-up password breach check** ([#989](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/989), [#991](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/991)) — Content Security Policy now allows the Have I Been Pwned k-anonymity range API so sign-up can detect compromised passwords instead of failing silently behind CSP.

## [3.6.2] - 2026-05-23

### Added

- **Marketing mobile demo videos** ([#987](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/987)) — PM Templates and QuickBooks feature pages now show autoplaying, looping mobile screen demos above existing screenshots, served from Supabase storage as compact MP4/WebM with poster fallbacks, native controls, and `prefers-reduced-motion` respect.
...
- **SPA route hard reload** ([#982](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/982)) — Hard reloads and deep links to authenticated app routes such as `/dashboard` and `/auth` no longer return Vercel's platform 404; routing fallback restored to the empty app shell after the v3.6.0 marketing prerender split.

- **CEVphantandocs CI on preview PRs** — Scope `CCEVphantanocs` Vercel builds to `main` via `docs/vercel.json` `ignoreCommand`; add `docs/postcss.config.js` so production docs builds resolve Tailwind PostCSS under `docs/`; filter `deployment-status` workflow to ignore `CECEVphantancs` deployment events.
...
- **QuickBooks customer contact sync** ([#914](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/914), [#960](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/960)) — QuickBooks customer imports and refreshes now capture email, phone, mobile, and fax contacts, show their QuickBooks provenance, and surface tap-to-call / tap-to-email actions on linked work orders without adding write-back to QuickBooks.
- **Public documentation site bootstrap (VitePress)** ([#908](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/908), [#956](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/956)) — `docs/` builds as a standalone static site (VitePress) for deployment to `CECEVphantanfo`; runbook updates in deployment and CI/CD docs; app footer links to published docs.
...
- **Public docs security** ([#956](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/956)) — Exclude `docs/ops/**` from the CCEVphantannfo VitePress build; remove Operations from public nav; README ops links use GitHub `blob/main`; tighten `Content-Security-Policy` `script-src` to `'self'` only (no `unsafe-inline` / `unsafe-eval`); add public vs internal authoring guidance; fix PM/RCA README links for GitHub.

### Fixed

- **Mobile work order details UX** ([#829](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/829), [#958](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/958)) — Sticky footer owns primary actions with PM checklist completion gating; hides duplicate Next action when the footer is visible; trims duplicate compact-summary rows from mobile details; mobile admin delete requires typing DELETE.
- **Mobile work order cost editor** ([#903](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/903), [#957](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/957)) — Touch-friendly stacked actions for labor, inventory, and manual lines; duplicate Cost Items chrome suppressed on small screens; inline validation deferred until Save or row edits; overflow clamps on notes/timeline/image carousel regions.

## [3.5.0] - 2026-05-15

### Added

- **QR PM template picker for untemplated equipment** ([#916](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/916), [#941](https://github.com/Columbia-Cloudworks-LLC/CCEVphantanull/941)) — QR work-order creation now surfaces a template chooser when scanned equipment has no default PM template, preserving the fast QR flow while preventing empty-template submissions.
...
- **QR PM template submit hardening** ([#941](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/941)) — Added validation and label-association guardrails to improve template selection reliability in QR PM work-order creation.

## [3.4.0] - 2026-05-14

### Added

- **Preview Edge secret sync (CI)** ([#906](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/906)) — `scripts/sync-supabase-secrets-from-1password.ps1` reads `ProjectRef` from each edge 1Password item, uses `supabase-write` / `SUPABASE_ACCESS_TOKEN`, validates placeholders and TOKEN_ENCRYPTION_KEY / KDF_SALT strength, compares SHA-256 digests in `-Check` mode (JSON list output trimmed for CLI banners), and applies via `supabase secrets set --env-file`. Non-prod **Secrets Fan-Out** Supabase job calls this script for `edge-env-preview-secrets`; auth probe loads the Supabase PAT from 1Password. Drift preflight helper simplified to `scope` + op item args. The `supabase-write` PAT is stored in 1Password as field `SUPABASE_ACCESS_TOKEN` (replacing legacy `credential`); `schema-drift-check` uses the same op:// reference. **Secrets Fan-Out (Non-Prod)** runs digest-only (`-Check`) on `push` to `preview` when this workflow or the sync script changes; the 6-hour UTC `schedule` applies preview Edge secrets (GitHub evaluates scheduled workflows from `main`, so the cron runs after this file exists on the default branch). Manual `workflow_dispatch` unchanged; production remains drift-check only until a future apply workflow.

- **QR scan feedback** ([#839](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/839)) — Live camera scans on `/dashboard/scan` prepare Web Audio on the Start camera gesture, set a short-lived session marker on successful decode, and play a synthesized ping plus vibration when `/qr/*` redirect access resolution completes. Upload-based decodes and direct QR opens stay silent. Development builds only: `/debug-scan-feedback` to audition the tone.

### Changed

- **Node.js 24 LTS runtime matrix** ([#931](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/931)) — `engines.node` is `24.x` (aligned with `@types/node` 24.x). GitHub Actions default to Node **24.x**; setup and CI docs describe the same supported line.

- **PageSEO / document metadata** — Removed the `react-helmet-async` dependency; `PageSEO` now updates `document.title` and head tags via a small scoped effect (tags marked `data-CEVphantanpage-seo` for cleanup). `HelmetProvider` was dropped from app and test providers.
- **GitHub Actions supply-chain hardening** ([#871](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/871)) — Third-party workflow actions and the shared 1Password composite action are pinned to full commit SHAs; the stale repository snapshot artifact workflow and Repomix configuration were removed.
...
- **CodeQL release gate reliability** ([#873](https://github.com/Columbia-Cloudworks-LLC/CEVphantanpull/873)) — CI grants the Security Scan job `actions: read` so CodeQL can read workflow-run metadata instead of failing with `Resource not accessible by integration`.
...
- **External customer contacts** — New `external_customer_contacts` table for contacts who are not registered CEVphantanusers (site managers, billing contacts, dispatchers). RLS grants org-member SELECT and org-admin mutate via join to the parent customer's organization. Full CRUD exposed through `ExternalContactsList` on the team detail page, gated by `canManageTeam`.
...
- **QuickBooks Import, Refresh, and Link flows** — Reworked `QuickBooksCustomerMapping` component with three distinct actions: Import from QuickBooks (creates a local `customers` row from the QB payload and links it to the team), Refresh from QuickBooks (updates only QB-sourced fields without overwriting CEVphantanonly edits like name, notes, or status), and Link Existing Account (attaches an existing local customer account). Legacy `quickbooks_team_customers` mapping is kept populated in parallel for rollback safety.
...
- **Mobile TopBar: CEVphantanmark instead of duplicate section titles** — On viewports below `md`, routes that already render a primary page heading (including `/dashboard`, `/dashboard/equipment`, work orders, inventory, fleet map, teams, reports, PM templates, audit log, settings, support, and organization) show the compact CCEVphantancon in `TopBar` instead of repeating the same section label, so the dashboard and equipment screens no longer show e.g. "Dashboard" or "Equipment" twice. Desktop breadcrumb-style labels are unchanged.
...
- Implemented the fix for [Issue #575](https://github.com/Columbia-Cloudworks-LLC/CEVphantanissues/575) in the shared image viewer so full-size images are no longer constrained by the old clipped container.
...
[Unreleased]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.32.0...HEAD
[3.32.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.31.0...v3.32.0
[3.31.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.30.0...v3.31.0
[3.30.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.29.1...v3.30.0
[3.29.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.29.0...v3.29.1
[3.29.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.28.0...v3.29.0
[3.28.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.27.0...v3.28.0
[3.27.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.26.1...v3.27.0
[3.26.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.26.0...v3.26.1
[3.26.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.25.31...v3.26.0
[3.18.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.17.0...v3.18.0
[3.11.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.11.2...v3.11.3
[3.11.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.11.1...v3.11.2
[3.11.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.11.0...v3.11.1
[3.11.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.10.0...v3.11.0
[3.10.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.9.3...v3.10.0
[3.9.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.9.2...v3.9.3
[3.9.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.9.1...v3.9.2
[3.9.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.9.0...v3.9.1
[3.9.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.7...v3.9.0
[3.8.7]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.6...v3.8.7
[3.8.6]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.5...v3.8.6
[3.8.5]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.4...v3.8.5
[3.8.4]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.3...v3.8.4
[3.8.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.2...v3.8.3
[3.8.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.1...v3.8.2
[3.8.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.8.0...v3.8.1
[3.8.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.6.4...v3.8.0
[3.6.4]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.6.3...v3.6.4
[3.6.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.6.2...v3.6.3
[3.6.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v3.6.1...v3.6.2
[2.8.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.7.1...v2.8.0
[2.7.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.7.0...v2.7.1
[2.7.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.6.0...v2.7.0
[2.6.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.5.2...v2.6.0
[2.5.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.5.1...v2.5.2
[2.5.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.5.0...v2.5.1
[2.5.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.4.0...v2.5.0
[2.4.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.10...v2.4.0
[2.3.10]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.9...v2.3.10
[2.3.9]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.8...v2.3.9
[2.3.8]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.7...v2.3.8
[2.3.7]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.6...v2.3.7
[2.3.6]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.5...v2.3.6
[2.3.5]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.4...v2.3.5
[2.3.4]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.3...v2.3.4
[2.3.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.2...v2.3.3
[2.3.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.1...v2.3.2
[2.3.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.3.0...v2.3.1
[2.3.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.2.4...v2.3.0
[2.2.4]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.2.3...v2.2.4
[2.2.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.2.2...v2.2.3
[2.2.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.2.1...v2.2.2
[2.2.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.2.0...v2.2.1
[2.2.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.1.3...v2.2.0
[2.1.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.1.2...v2.1.3
[2.1.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.1.1...v2.1.2
[2.1.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.1.0...v2.1.1
[2.1.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v2.0.0...v2.1.0
[2.0.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.8.1...v2.0.0
[1.8.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.8.0...v1.8.1
[1.8.0]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.13...v1.8.0
[1.7.13]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.12...v1.7.13
[1.7.12]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.11...v1.7.12
[1.7.11]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.10...v1.7.11
[1.7.10]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.9...v1.7.10
[1.7.9]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.8...v1.7.9
[1.7.8]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.7...v1.7.8
[1.7.7]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.4...v1.7.7
[1.7.4]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.3...v1.7.4
[1.7.3]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.2...v1.7.3
[1.7.2]: https://github.com/Columbia-Cloudworks-LLC/CEVphantancompare/v1.7.1...v1.7.2
[1.7.1]: https://github.com/Columbia-Cloudworks-LLC/CEVphantanreleases/tag/v1.7.1
